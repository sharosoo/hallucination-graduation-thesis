...
1.  **SE / Energy 의 prompt-level baseline 재현**. Farquhar 등(2024) 의 SE 와 Ma 등(2025) 의 Semantic Energy 가 본 데이터 (TruthfulQA + HaluEval-QA) 의 prompt-level hallucination 탐지에서 어떤 AUROC 를 보이는지 측정하고, SE 원논문 보고치와 일치 여부를 확인한다.
...
- **Mid–high decile 에서는 corpus 추가가 큰 차이 없음 또는 미세 negative**. decile 40–80 영역에서 corpus 추가 lift 는 $`-0.009`$   $`0.000`$ 사이. 이 영역의 hallucination 은 SE / Energy 만으로도 잘 탐지되어 (AUROC 0.85–0.92) corpus 의 추가 정보가 새 contribution 을 거의 못한다.
...
GBM (with corpus) 가 두 dataset 모두에서 1위이다. HaluEval-QA 에서 corpus 추가 lift 는 +0.002 (0.815 → 0.817) 로 작고, TruthfulQA 에서는 +0.012 (0.951 → 0.963) 로 더 크다. 다만 TruthfulQA 의 is_hard 라벨 비율 (0.97) 이 매우 비대칭임에 주의해야 한다 — 본 논문의 token-overlap 기반 매칭 (free-sample $`\ge`$ 50% token overlap with right_answer / best_answer / correct_answers list) 은 의미적으로 동등한 paraphrase 를 충분히 잡지 못해 TruthfulQA 의 다양한 정답 표현 (예: “Cardiff”, “the Welsh capital”, “a city in Wales”) 을 hard 로 잘못 분류하는 노이즈가 있다. NLI-기반 매칭으로 라벨링하면 이 노이즈가 줄어들 가능성이 높으며, 이는 §결론 한계 항목에서 future work 로 명시한다. 그럼에도 GBM (with corpus) 가 두 dataset 모두에서 안정적인 1위라는 사실은 본 논문 main 결과가 단일 dataset 에 의존하지 않음을 보여준다.
...
3.  **평가 단위 분리의 명시적 적용 + 분석 framework**. SE 는 prompt-level, Semantic Energy 는 cluster $`\to`$ prompt collapse, logit diagnostic 은 candidate-level 신호임을 본문, 표, 그림 전체에 일관되게 적용하여 단위 혼동에서 비롯된 잘못된 비교를 차단하였다. 본 논문의 기여는 새로운 알고리즘이 아니라 (i) prompt-level main + candidate-row sensitivity 의 평가 단위 분리, (ii) corpus signal 의 한계 효용을 통계적 유의성과 영역별 분포로 분해하는 분석 절차이다.
