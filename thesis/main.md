...
<table>
<caption>종합 baseline 결과 (단일 자동화 실행 산출, spaCy entity extractor). 후보 행 11<span>,</span>630개, 학습 라벨은 데이터셋 annotation의 <code>is_hallucination</code>. learned fusion 계열은 prompt-grouped fold 분리로 평가하여 같은 prompt의 두 후보가 fold를 가로지르지 않게 강제하였다.</caption>
<thead>
<tr>
...
<td style="text-align: center;"><strong>0.555</strong></td>
<td style="text-align: center;">0.667</td>
...
- 본 논문이 제안하는 corpus-bin weighted fusion 은 AUROC **0.601** 로 모든 baseline 중 1위이다. 정의상 단순한 선형 구조 위에서 corpus support bin 마다 다른 가중치를 학습하는 것만으로, 비선형 gradient boosting (0.541) 을 +0.060 차이로 능가한다.
...
|:---|:--:|:--:|:--:|
| 00–10 | 6,491 | **0.623 \[0.607, 0.640\]** | **$`+0.0305`$ \[$`+0.0277`$, $`+0.0335`$\]** |
| 10–20 | 1,843 | **0.621 \[0.589, 0.652\]** | **$`+0.0275`$ \[$`+0.0224`$, $`+0.0327`$\]** |
| 20–30 | 1,127 | **0.652 \[0.613, 0.692\]** | **$`+0.0337`$ \[$`+0.0277`$, $`+0.0405`$\]** |
| 30–40 | 701 | **0.564 \[0.510, 0.616\]** | $`+0.0018`$ \[$`-0.0048`$, $`+0.0086`$\] |
| 40–50 | 495 | 0.384 \[0.322, 0.443\] | $`-0.0034`$ \[$`-0.0117`$, $`+0.0040`$\] |
| 50–60 | 322 | 0.481 \[0.406, 0.556\] | $`-0.0049`$ \[$`-0.0143`$, $`+0.0048`$\] |
| 60–70 | 253 | **0.597 \[0.508, 0.678\]** | **$`+0.0352`$ \[$`+0.0156`$, $`+0.0552`$\]** |
| 70–80 | 208 | **0.624 \[0.532, 0.716\]** | **$`+0.0443`$ \[$`+0.0214`$, $`+0.0701`$\]** |
| 80–90 | 137 | 0.386 \[0.286, 0.500\] | $`-0.0499`$ \[$`-0.0905`$, $`-0.0156`$\] |
| 90–100 | 53 | 0.417 \[0.250, 0.625\] | $`-0.0199`$ \[$`-0.0890`$, $`+0.0469`$\] |

Corpus support decile별 corpus-bin weighted fusion의 paired 통계 95% 신뢰구간 (prompt-grouped bootstrap, n=2,000). win-rate $`> 0.5`$ 가 “환각 후보가 더 위험하게 평가됨”을 의미. spaCy entity extractor 기반.

</div>

세 가지 패턴이 두드러진다.

- **Decile 00–30 sweet spot (n=9,461, 81%)**: 세 decile 모두 paired win-rate 신뢰구간이 \[0.589, 0.692\] 범위에서 0.5보다 통계적으로 유의미하게 높다. spaCy entity extractor 기반에서는 다중 entity 가 추출되면서 entity_frequency_min 이 0 이 되기 쉬워 후보 데이터의 81% 가 낮은 corpus support 구간에 속하며, 이 *대량 영역* 에서 본 결합기가 가장 강하게 작동한다.

- **Decile 60–80 보조 영역 (n=461, 4%)**: 적은 표본이지만 win-rate \[0.508, 0.716\] 로 유의미한 우위. 중간 corpus support 영역에서도 결합기가 일관 작동.

- **Decile 40–60, 80–100 비유의 또는 역방향 (n=1,007, 9%)**: 표본이 적고 (각 53–495) 신뢰구간이 0.5를 포함하거나 0.5 보다 낮은 방향에 머무른다. 특히 decile 80–90 (mean delta $`-0.05`$ CI excludes 0) 은 통계적으로 *잘못된 방향* 으로 작동함을 보여, 매우 높은 corpus support 영역에서 결합기 학습이 표본 부족으로 불안정함을 시사한다.

종합 수준의 비교에서 corpus-bin weighted fusion (AUROC 0.601) 은 선형 global fusion (logistic regression, AUROC 0.372 — 무작위 0.5 이하의 역방향 결합기) 대비 AUROC **+0.229 \[+0.219, +0.239\]**, AUPRC +0.138 \[+0.131, +0.146\] 로 신뢰구간이 0을 포함하지 않는다. 비선형 sklearn gradient boosting (no corpus, AUROC 0.541) 과의 비교에서도 +0.060 차이로 본 논문 제안 결합기가 종합 1위이다. 본 논문 제안의 우위는 (i) decile 별 paired 분해에서 다수 구간 통계적 우위 (본 절 표 <a href="#tab:bootstrap_ci" data-reference-type="ref" data-reference="tab:bootstrap_ci">4.3</a>), (ii) calibration 지표의 약 2.5배 개선 (Brier 0.243 vs 0.278, ECE 4.9% vs 12.1%), (iii) HaluEval-QA 도메인에서 0.620 의 강한 일반화 (§<a href="#sec:lodo" data-reference-type="ref" data-reference="sec:lodo">4.6</a>) 가 결합된 결과이다. 본 절의 decile 별 분해는 그 조건부 우위가 *어느 corpus support 구간에서 발생하는지* 를 정밀하게 보여준다.
...
|:---|:--:|:--:|:--:|:--:|:--:|
| 00–10 | 6,491 | **0.637** | 0.567 | 0.553 | 0.318 |
| 10–20 | 1,843 | **0.634** | 0.535 | 0.514 | 0.321 |
| 20–30 | 1,127 | **0.635** | 0.484 | 0.474 | 0.297 |
| 30–40 | 701 | **0.542** | 0.488 | 0.490 | 0.397 |
| 40–50 | 495 | 0.472 | **0.519** | 0.490 | 0.498 |
| 50–60 | 322 | **0.501** | 0.487 | 0.483 | 0.452 |
| 60–70 | 253 | **0.536** | 0.482 | 0.476 | 0.443 |
| 70–80 | 208 | **0.601** | 0.481 | 0.473 | 0.510 |
| 80–90 | 137 | 0.496 | **0.514** | 0.500 | 0.503 |
| 90–100 | 53 | **0.529** | 0.476 | 0.518 | 0.391 |

Method $`\times`$ corpus support decile AUROC (spaCy 기반). 굵은 글씨는 decile별 1위.
...
| decile | n | corpus-bin weighted fusion | gradient boosting (no corpus) | random forest (no corpus) | logit-diagnostic |
|:---|:--:|:--:|:--:|:--:|:--:|
| 00–10 | 6,491 | **0.623** | 0.566 | 0.555 | 0.330 |
| 10–20 | 1,843 | **0.621** | 0.527 | 0.515 | 0.336 |
| 20–30 | 1,127 | **0.652** | 0.476 | 0.469 | 0.301 |
| 30–40 | 701 | **0.564** | 0.482 | 0.484 | 0.396 |
| 40–50 | 495 | 0.384 | **0.515** | 0.487 | 0.493 |
| 50–60 | 322 | **0.481** | 0.479 | 0.474 | 0.451 |
| 60–70 | 253 | **0.597** | 0.474 | 0.470 | 0.434 |
| 70–80 | 208 | **0.624** | 0.475 | 0.466 | 0.503 |
| 80–90 | 137 | 0.386 | **0.508** | 0.493 | 0.497 |
| 90–100 | 53 | 0.417 | 0.464 | **0.510** | 0.376 |

Method $`\times`$ corpus support decile paired win-rate (spaCy 기반). 0.5가 무작위. 굵은 글씨는 decile별 1위.
...
- **Decile 00–30: 대량 sweet spot** (n=9,461, 81%). spaCy entity extractor 가 다중 entity 를 추출하면서 entity_frequency_min=0 이 더 빈번해져 본 데이터의 81% 가 낮은 corpus support 구간에 속한다. 이 영역에서 corpus-bin weighted fusion 이 AUROC 0.634–0.637, paired win-rate 0.62–0.65 로 모든 방법 중 1위이다 — corpus 가 거의 모르는 entity 가 포함된 답에서 본 결합기가 가장 강하게 작동한다.

- **Decile 70–80** (n=208). corpus-bin weighted fusion 이 AUROC 0.601, win-rate 0.624 로 두 번째 강한 영역. 중간–상위 corpus support 에서도 결합기가 일관 작동.

- **Decile 40–60, 80–100: 약화 영역** (n=1,007). 표본이 적고 (각 53–495) corpus-bin weighted fusion 의 win-rate 가 0.5 근처에 모이거나 (0.481, 0.386, 0.417) 잘못된 방향으로 떨어진다. 매우 높은 corpus support 영역의 표본 부족으로 결합기 학습이 불안정해지는 패턴.

- **Logit-diagnostic CHOKE 패턴**. logit-diagnostic 의 paired win-rate 가 decile 00–30 에서 0.30–0.34 로 가장 강한 역전을 보인다. 환각 답이 정답 답보다 더 자신있게 생성되는 현상은 corpus 가 거의 모르는 답 (대량 영역) 에서 가장 두드러진다.
...
|:-------------------------------|:---------:|:----------------:|:---------------:|
| **corpus-bin weighted fusion** | **0.601** |    **0.620**     |    **0.540**    |
...
| Method                         |   Brier   |    ECE    |
|:-------------------------------|:---------:|:---------:|
| **corpus-bin weighted fusion** | **0.243** |   0.049   |
