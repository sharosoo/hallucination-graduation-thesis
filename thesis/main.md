...
신호가 *어떤 sample 의 정답 확률을 직접 예측하는 신호* 로는 약하다는
점을 의미한다.
...
에서 0.793 로 $\Delta 0.150$ 변동하며, ECE (calibration 오차) 도 구간
사이에서 0.07--0.28 의 큰 변동을 보인다. 즉 SE 의 *ranking 분리력*
(AUROC) 과 *확률 calibration* (ECE) 모두 corpus 영역에 따라 달라진다.
이는 SE 의 신뢰도가 corpus 영역에 의해 조절되는 (effect modification)
패턴이 단일 측정 인공물이 아니라 다중 평가 지표에서 일관되게 관찰되는
본질적 특성임을 시사한다.
...
modification 패턴이다. 즉 corpus 신호는 "어느 영역에서 SE 를 신뢰해야
하는가" 라는 *메타 정보* 로 작용하며, "개별 답변이 환각인가" 를 직접
예측하는 정보로는 약하다. 본 비대칭은 이 두 역할의 차이를 정량화한
결과이며, 단순한 측정 인공물 (예: ranking metric 의 분포 의존성) 이
아니라 corpus 환경에 따른 모델 행동 변화를 반영한다고 해석된다. 다만 본
절은 부분 분석이며, 가능한 다른 메커니즘 (예: fusion 모델의 비선형성
한계, 10분위 이산화 효과) 의 완전한 배제는 향후 과제로 남긴다.
...
채택되어 왔다 (예: Min 등 2025[@qucorag] 의 QuCo-RAG). 그러나 이 proxy
