...
(2023)[@kandpal2023longtail] 이 사전학습 corpus 에서 (질문 entity, 답변
entity) 쌍의 동시 등장 횟수가 단일 entity 빈도보다 LLM 의 사실 정답률을
더 잘 예측한다는 것을 정량적으로 보인 결과와 직접적으로 맞닿아 있다.
...
*Semantic Uncertainty* 논문에 있다. Kuhn 등은 자연어 생성에서 의미적
등가 (서로 다른 표현이 같은 뜻을 가질 수 있다) 라는 linguistic
invariance 를 명시적으로 다루는 entropy 를 정의하여, 후속 hallucination
detection 연구의 이론적 토대를 제공했다. Farquhar 등
(2024)[@farquhar2024] 의 Semantic Entropy 는 이를 hallucination
detection 으로 구체화한 후속 연구이다. 본 논문은 후자의 framing 을
따르되, 원형 방법론의 기원을 명시하여 방법의 계보를 분명히 한다.
...
등 (2023) 은 The Pile 과 ROOTS 사전학습 corpus 에서 (질문 entity, 답변
entity) 쌍의 동시 등장 횟수를 산출하고, 이 횟수와 LLM 의 정답률 사이에
강한 양의 상관이 있음을 보였다. 특히 단일 entity 빈도보다 entity 쌍 동시
등장이 정답률을 더 잘 예측한다는 점을 명시적으로 보고했다. Mallen 등
(2023) 은 답변 entity 의 Wikipedia 페이지뷰 (popularity) 를 corpus
노출의 proxy 로 삼아 PopQA 데이터셋의 sample 을 popularity bin 으로
나누고, popularity 가 낮은 bin 에서 LLM 정답률이 급격히 떨어지는 패턴을
보였다. Razeghi 등 (2022)[@razeghi2022termfreq] 은 few-shot numerical
reasoning 태스크에서 사전학습 term 빈도와 모델 정답률 사이의 강한 양의
상관을 정량적으로 보였다. Kandpal 등 (2023) 의 factual QA, Mallen 등
(2023) 의 popularity 기반 분석과 함께, 사전학습 corpus 노출 정도와 모델
성능의 양의 관계가 특정 task 를 넘어 reasoning 전반에 걸쳐 안정적으로
나타난다는 점을 뒷받침한다. 본 연구가 AUROC 수준으로 한 단계 더 끌어올려
분석한 기반이 된다.
...
AUC 0.89 를 보고했다 (peer review 미완료 preprint, 수치의 독립 검증
필요). SEReDeEP (Wang, 2025)[@wang2025seredeep] 는 RAG 환경에서 semantic
entropy 와 context-parameter knowledge 균형 신호를 결합하여 환각 예측
정확도 향상을 보고했다 (peer review 미완료 preprint, 수치 보수적 해석
필요). Valentin 등 (2024)[@valentin2024] 은 환각 탐지 점수를 모델 내부
score attribute 에 조건부로 calibrate 하는 다중 점수 프레임워크를
제안했다.
...
선행 연구와의 차이를 명시하면 다음과 같다. Kandpal 등
(2023)[@kandpal2023longtail] 은 corpus 노출과 모델 정답률, Mallen 등
(2023)[@mallen2023whennot] 은 entity popularity 와 정답률의 관계를
보였다. 본 연구는 동일한 corpus 신호 (entity 빈도, entity 쌍 동시 등장)
를 사용하되, 측정 대상을 *모델 정답률* 이 아니라 *환각 탐지 신호의
AUROC* 로 한 단계 더 높은 분석 수준으로 끌어올렸다. 두 지표는 자동으로
비례하지 않는다. 정답률이 높은 영역과 낮은 영역에서도 Semantic Entropy
의 AUROC 는 동일할 수 있고, 정답률이 균일한 영역에서도 AUROC 가 영역마다
다를 수 있다. 또한 동일 corpus 신호의 두 역할 --- 탐지기 입력 변수와
영역별 평가 척도 --- 사이의 비대칭은 두 선행 연구에는 정의 자체가 없는
분석 축이다. 본 연구가 일곱 가지 corpus 신호 단위 (entity 빈도, entity
co-occurrence, baseline, question-answer bridge, 답변 3-gram / 5-gram,
3-gram 미등장) 를 같은 평가 척도 위에서 비교한 것도 두 선행 연구의 범위
밖이다.
...
reference="tab:se_corpus_corr"} 는 Semantic Entropy 와 6개 corpus 신호
사이의 Spearman $\rho$ 를 보고한다. 모든 절대값이 0.02--0.16 범위에
머물러 두 신호군이 거의 직교하는 것에 가깝다. "corpus 신호의 fusion lift
가 작은 이유는 SE 와 이미 상관이 높아 한계 정보가 작기 때문" 이라는
가설은 이 데이터에서 강하게 지지되지 않는다. corpus 신호가 SE 와
무관함에도 불구하고 fusion 입력으로 +0.008 만 기여한다는 사실은, corpus
신호가 *개별 sample 의 정답 확률을 직접 예측하는 신호* 로는 약하다는
점을 의미한다.
...
표준화된 입력 ($-$Semantic Entropy, entity_pair_cooccurrence_axis,
그리고 두 변수의 곱) 위에서 logistic regression 을 prompt 단위 5-fold
GroupKFold OOF 로 학습한다. 두 모델을 비교한다. 첫째는 main effect 만
(SE + corpus), 둘째는 interaction term 까지 포함 (SE + corpus +
SE$\times$corpus). 표준화 계수는 SE $+1.13$, corpus $-0.35$, interaction
$-0.37$ 로, interaction 의 절대 크기가 corpus main effect 와 같은
수준이다. 그러나 OOF AUROC 는 0.7585 (no interaction) $\to$ 0.7590 (with
interaction) 로 +0.0005 만 변동한다. interaction 자체는 통계적으로 0 이
아니지만, 예측 분리력을 끌어올리는 데는 기여하지 않는다. corpus 신호가
SE 의 *개별 답변 단위 확률* 을 조절한다기보다는 SE 가 신뢰할 수 있는
*영역의 경계* 를 정한다는 해석에 부합한다.

##### 3) Corpus 영역별 calibration.

표 [\[tab:per_bin_calibration\]](#tab:per_bin_calibration){reference-type="ref"
reference="tab:per_bin_calibration"} 는 entity co-occurrence 10분위 구간
안에서 Semantic Entropy 의 sample 단위 정답률, AUROC, Brier, ECE 를 함께
보고한다. corpus 가 가장 부족한 구간 (00--10) 의 정답률은 0.27 인데 가장
풍부한 구간 (70--80) 은 0.38 로, 정답률이 약 1.4배 높다. AUROC 는 0.643
에서 0.793 으로 $\Delta 0.150$ 변동하며, ECE (calibration 오차) 도 구간
사이에서 0.07--0.28 의 큰 변동을 보인다. 즉 SE 의 *ranking 분리력*
(AUROC) 과 *확률 calibration* (ECE) 모두 corpus 영역에 따라 달라진다.
이는 SE 의 성능 자체가 corpus 영역에 의해 조절되는 effect modification
패턴이며, 특정 평가 지표에서만 나타나는 분석 절차의 부산물이 아니라 다중
평가 지표에서 일관되게 관찰되는 본질적 특성임을 보여준다.
...
다만 이 수치는 다섯 데이터셋을 합산한 전체 표본 결과이다. 데이터셋별로는
BioASQ 2.92배, TriviaQA 0.99배이며, NQ-Open 에서는 부호가 역전된다
(entity 빈도 0.372 \> entity co-occurrence 0.250). 전체 표본 1.88배는
BioASQ 의 높은 비율이 NQ-Open 의 역전을 상쇄하여 형성된 종합 통계로,
"모든 도메인에서 성립한다" 는 주장은 아니다
(표 [\[tab:per_dataset_delta\]](#tab:per_dataset_delta){reference-type="ref"
reference="tab:per_dataset_delta"} 참조).
...
를 약 1.88배 (전체 표본 기준) 크게 보인다는 점 --- 은 본 결과에서
도출된다. 다만 이 비율은 데이터셋별로 0.67--2.92배 범위에서 변동하며,
max$-$min 통계량의 특성상 이상 구간 (NQ-Open 20--30 구간 AUROC 0.266 등)
에 민감하다. 따라서 범위 표현인 1.5\~1.9배가 더 안정적인 요약이다.
...
이러한 proxy 사용은 다음과 같은 근거로 정당화된다. Web-scale 사전학습
corpus (Dolma, RedPajama, C4 등) 는 Common Crawl 기반의 공통 source 를
상당 부분 공유하므로, 하나의 transparent corpus 에서 산출한 통계가 다른
LLM 의 학습 노출에 대한 합리적 proxy 역할을 할 수 있다는 가정은 선행
연구에서도 널리 채택되어 왔다 (예: Min 등 2025[@qucorag] 의 QuCo-RAG).
그러나 이 proxy 대체는 형식적으로 보장되지 않으며, 본 논문 결과의 해석에
두 가지 제약을 가한다.

첫째, corpus 뒷받침이 큰 sample 에서 Semantic Entropy AUROC 가 높게
관찰되는 결과는 "모델이 학습해서 잘 안다" 라는 인과 진술이 아니라,
"Dolma 와 같은 web 분포에서 자주 등장하는 사실은 Qwen 학습에서도 자주
등장했을 가능성이 높다" 라는 간접 추론을 통해서만 해석될 수 있다. 둘째,
entity co-occurrence 가 entity 빈도보다 약 1.88배 큰 분해를 보인다는
비율 또한 Dolma 색인에 한정된 관찰이며, Qwen 의 실제 corpus 나 다른
모델의 corpus 에서는 두 단위의 비율이 달라질 수 있다.

따라서 본 결과는 corpus 빈도와 환각 사이의 상관 관찰에 한정되며, 모델
학습 가능성에 대한 인과 진술로 해석해서는 안 된다. 이 한계는 모델
제공자가 사전학습 corpus 를 공개하기 전까지 본 연구 설계에 내재하며,
후속 연구가 corpus 색인을 모델별 실제 학습 데이터로 교체하기 전에는 수치
자체의 외부 타당성에 분명한 제약이 있다.

위 한계 (proxy corpus index 사용) 와 아래 단일 모델 평가는 결론의 외부
타당성을 근본적으로 제한하는 두 핵심 한계이다. 이하 항목은 실험 절차와
서술 수준의 추가 제약이다.

1.  **\[핵심 한계\] 단일 모델 평가.** 본 연구는 Qwen2.5-3B 단일 모델에서
    수행되었다. corpus 신호의 구간별 변동 패턴이 Llama, Mistral, Gemma
    계열 모델에서도 같은 방향으로 나타나는지는 직접 검증되지 않았다.
    Farquhar 등 (2024)[@farquhar2024] 에서 서로 다른 LLM 계열 사이에서도
    Semantic Entropy 의 패턴이 유사하게 관찰되었다는 점은 일반화
    가능성을 시사하나, 이는 향후 연구로 남긴다.
...
5.  **AUROC range 차이의 표본 변동성.** 약 1.88배라는 비율은 단일
    ...
    단위 bootstrap 95% 신뢰구간을 §[4.6](#sec:axis){reference-type="ref"
...
8.  **Corpus 신호의 entity 추출 범위.** 본 논문의 corpus 신호는 질문
    ...
    다시 산출한 결과는 §[4.6](#sec:axis){reference-type="ref"
    reference="sec:axis"} 에 함께 보고하였으며, Semantic Entropy 기준
    비율은 1.88 에서 2.20 으로 증가한다. 다만 부분표본 (n=2,095) 의 선택
    편향은 §[4.6](#sec:axis){reference-type="ref" reference="sec:axis"}
    에 명시하였다.
...
10. **NLI 임계값 0.5 의 민감도 미검증.** 본 논문은 정답 라벨을 양방향
    ...
    의존하는지는 후속 검증이 필요하다. 참고로 임계값 변화는 정답 라벨
    자체의 분포를 바꾸므로 AUROC 절대값과 range 모두에 영향을 미칠 수
    있으며, 향후 LLM-as-judge 비교 검증과 함께 수행하는 편이 효율적이다.
...
14. **선행 연구 인용의 검증 한계.** 본 논문이 인용한 2025년 arXiv
    preprint 일부 (Singha 2025[@singha2025eclipse], Wang
    2025[@wang2025seredeep], Zhang 2025[@zhang2025corpus], Min
    2025[@qucorag] 등) 는 본 논문 제출 시점 (2026년 2월) 기준 peer
    review 를 거치지 않은 작업이며, 보고된 수치의 재현 가능성은 독립적
    검증이 필요하다. 특히 단일 저자 preprint 의 self-reported 개선폭은
    보수적으로 해석한다. 본 논문 §2 의 인용은 갈래 정리 (배경 / 동기
    부여) 목적에 한정하며, 본 논문 메인 결과의 비교 기준 (Farquhar 2024
...
유의함을 보여주지만, 효과 크기의 정밀도는 낮으며, NQ-Open 에서는 비율이
역전된다 (entity 빈도 \> entity co-occurrence). 따라서 전체 표본 수치는
데이터셋 종합 통계로 해석하며, 도메인별 최적 corpus 신호 단위는 후속
연구 과제이다. 본 결과는 corpus 빈도와 환각 사이의 상관 관계 관찰로
해석한다 (현 색인이 모델의 실제 사전학습 corpus 와 다르므로 인과 해석은
불가).
