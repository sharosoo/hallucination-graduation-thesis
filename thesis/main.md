...
ChatGPT 와 Gemini 등으로 대표되는 LLM 은 추론 및 질의응답에서 높은
성능을 보이지만, 사실과 다르거나 근거가 없는 답변, 즉
환각(hallucination)을 빈번하게 생성한다. 의료, 법률, 과학과 같은 고위험
도메인에서 환각은 중대한 오류로 이어질 수 있으며, 이에 따라 LLM 출력의
신뢰성을 평가하는 일반적인 방법론은 중요한 연구 과제로 자리 잡았다.

환각 탐지의 주요 접근법 가운데 하나는 불확실성 정량화이다. Farquhar 등
(2024) 가 제안한 Semantic Entropy 는 동일 sample 에 대해 N개의 답변을
생성한 뒤 NLI 기반 의미 cluster 의 확률 분포 엔트로피로 모델의 의미적
불확실성을 측정한다. Semantic Entropy 는 모델, 데이터셋, 태스크에 걸쳐
강건하게 일반화되며 외부 지식 없이 동작한다. Ma 등 (2025) 의 Semantic
Energy 는 확률 대신 logit 을 직접 활용해 Semantic Entropy 가 포착하지
못하는 모델 지식 자체에 대한 불확실성까지 추정한다.

그러나 두 방법은 모두 모델 내부 신호에만 의존하며, 개별 sample 의 corpus
support 조건, 즉 해당 사실이 사전학습 corpus 에 얼마나 자주 등장했는지에
따라 탐지 성능이 어떻게 달라지는지를 충분히 분석하지 않았다. 모델 전체
평균 AUROC 만 보면 탐지 신호가 어느 corpus support 영역에서 안정적이고
어느 영역에서 그렇지 않은지를 구분하기 어렵다. corpus 통계량을 환각
탐지에 활용한 선행 연구 가 있으나, 이들은 corpus 신호 자체를 탐지 신호로
사용하는 데 머물렀다. corpus 신호의 단위가 Semantic Entropy, Semantic
Energy, 답변 단위 logit 통계 등 환각 탐지 신호의 구간별 변동에 어떻게
작용하는지를 비교한 연구는 보고된 바 없다.

## 연구 목적

본 논문의 핵심 질문은 다음과 같다. 환각 탐지 신호(Semantic Entropy,
Semantic Energy, 답변 단위 logit 통계) 의 corpus 뒷받침 조건 의존성은
corpus 신호의 단위 (entity 빈도, entity co-occurrence, n-gram) 에 따라
어떻게 다르게 나타나는가.

이 질문에 답하기 위해 세 가지 세부 목표를 설정한다. 첫째, Farquhar 등
(2024) 와 동일한 5개 데이터셋과 답변 단위 NLI 정답 판정 절차 위에서
Qwen2.5-3B 모델로 Semantic Entropy, Semantic Energy, 답변 단위 logit
통계의 재현성을 검증한다. 둘째, entity, entity co-occurrence,
question-answer bridge, 답변 n-gram 등 단위가 다른 corpus 신호를
설계하고, 각 신호가 단독 환각 탐지기로 사용될 때 분리력이 제한적이라는
결과를 정량화한다. 셋째, 같은 corpus 신호로 sample 을 10분위 구간으로
나누었을 때 세 환각 탐지 신호 (Semantic Entropy, Semantic Energy, 답변
평균 음의 로그우도) 의 구간별 AUROC 가 어떻게 변동하는지를 분석하고,
corpus 신호의 단위가 변동 폭에 미치는 영향을 규명한다.

##### 본 논문의 기여.

기여는 다음 세 가지이다. (1) 환각 탐지 신호가 corpus support 조건에 따라
어떻게 달라지는지를 corpus 신호의 단위별로 비교하는 분석 절차를
제시한다. (2) 본 표본에서 entity co-occurrence 신호로 sample 을 나누었을
때의 구간별 AUROC 변동 폭이 $`\Delta`$<!-- -->0.150 로, 단일 entity 빈도
신호의 변동 폭 $`\Delta`$<!-- -->0.080 보다 약 1.88배 크다는 결과를 단일
조건에서 산출한 점추정치로 보고한다. 신뢰구간과 모델·corpus 일반화는
§<a href="#sec:limitations" data-reference-type="ref"
data-reference="sec:limitations">5.2</a> 에서 다룬다. (3) corpus 신호의
설계 목표를 탐지기 입력 변수가 아니라 조건부 평가 도구로 다시 설정하는
관점을 제시한다.
...
신호로 활용하는 연구는 아직 초기 단계이며, 어떤 단위 (entity, 두 entity
사이의 관계, phrase) 의 corpus 통계 신호가 탐지 신호와 어떻게
...
있음을 보인다. 그러나 corpus 지지 신호의 역할 — 입력 변수로서의 한계와
sample 분할 신호로서의 효용 — 을 구분해 분석한 연구는 충분하지 않다. 본
논문은 corpus 통계 신호를 fusion 입력에 추가했을 때의 성능 변화를
정량화하는 한편, 같은 신호로 sample 을 나누었을 때 환각 탐지 신호의
AUROC 가 어떻게 달라지는지를 비교함으로써 이 공백을 메운다.
...
신호의 구간별 AUROC 변동 폭으로 정의한다. 변동 폭이 클수록 그 corpus
신호로 sample 을 나누었을 때 환각 탐지 신호의 성능이 corpus 조건에 따라
더 큰 차이를 보인다. 본 논문의 핵심 결과는 이 변동 폭이 corpus 신호의
단위에 따라 어떻게 달라지는지를 비교한 것이다.
...
data-reference="sec:axis">4.5</a>) 에서 가장 큰 구간별 AUROC 변동 폭을
...
sample 을 나누어 분석했을 때의 구간별 변동 폭은 서로 다른 지표이다. 본
논문 핵심 결과의 출발점이 바로 이 비대칭이다.
...
제한적인 반면, 같은 신호로 sample 을 분할했을 때의 구간별 AUROC 변동
폭은 큰 비대칭이 관찰되며 이는 다음 절의 결과로 이어진다.
...
산출하고, 그 최댓값과 최솟값의 차이
$`\Delta = \mathrm{AUROC}_{\max} - \mathrm{AUROC}_{\min}`$ 를 변동
폭으로 측정한다. 본 논문의 핵심 결과는
...
모두 entity co-occurrence 신호로 분할했을 때의 변동 폭이 entity 빈도
신호로 분할했을 때보다 크며, Semantic Entropy 와 Semantic Energy 에서 그
차이가 뚜렷하게 나타난다. Semantic Entropy 의 변동 폭은 entity
co-occurrence 신호에서 <span class="math inline"><em>Δ</em></span>0.150,
entity 빈도 신호에서 <span class="math inline"><em>Δ</em></span>0.080
이다. Semantic Energy 도 비슷한 패턴을 따른다 (<span
class="math inline"><em>Δ</em></span>0.144 와 <span
class="math inline"><em>Δ</em></span>0.077). 답변 평균 NLL 은 두 신호
모두에서 변동 폭이 작다 (<span
...
##### Entity co-occurrence 신호의 구간별 변동 폭.

Entity co-occurrence 신호로 sample 을 분할하면, Semantic Entropy 의 변동
폭은 $`\Delta`$<!-- -->0.150, Semantic Energy 의 변동 폭은
$`\Delta`$<!-- -->0.144 로 산출된다. 본 표본에서 비교한 일곱 가지 corpus
신호 가운데 변동 폭이 가장 큰 값이다. sample 단위 부트스트랩 (반복
500회) 으로 산출한 95% 신뢰구간은 Semantic Entropy 변동 폭이
0.121–0.211, entity 빈도 신호의 변동 폭이 0.058–0.149 이다. 두 변동 폭의
차이는 95% 신뢰구간 \[+0.002, +0.117\] 으로 0 을 포함하지 않으며,
부트스트랩 반복의 97.6% 에서 양수로 관찰된다. Semantic Energy 에서도
차이가 \[+0.004, +0.109\] 로 98.0% 가 양수이다. 즉 entity co-occurrence
신호로 분할했을 때의 변동 폭이 entity 빈도 신호로 분할했을 때보다 크다는
결과는 표본 변동성을 통제한 뒤에도 통계적으로 유지된다. corpus 가 가장
부족한 구간 (00–10) 에서 Semantic Entropy 는 0.643, Semantic Energy 는
0.667 이며, corpus 가 가장 풍부한 구간 (70–80) 에서 Semantic Entropy 는
0.793, Semantic Energy 는 0.811 로 측정되었다. 다만 구간이 한 단계씩
올라갈 때마다 AUROC 가 단조 증가하지는 않으며, 중간 구간에서는 일시적인
하락과 회복이 관찰된다
(그림 <a href="#fig:axis_decomp" data-reference-type="ref"
...
##### Entity 빈도 신호의 구간별 변동 폭.

단일 entity 빈도 (entity frequency) 신호로 sample 을 분할했을 때의 변동
폭은 Semantic Entropy 기준 $`\Delta`$<!-- -->0.080, Semantic Energy 기준
$`\Delta`$<!-- -->0.077 로 본 표에서 가장 작다. 본 연구의 baseline
corpus 신호 (entity 빈도와 entity co-occurrence의 평균) 도 변동 폭이
$`\Delta`$<!-- -->0.082–0.086 으로 entity 빈도 신호와 거의 같은
수준이다. entity co-occurrence 신호의 큰 변동 폭이 entity 빈도 신호의
작은 변동 폭과 평균되며 희석된 결과로 해석되며, corpus 신호를 설계할 때
두 단위를 따로 보고해야 한다는 점을 시사한다. 평균화의 영향은 변동 폭뿐
아니라 단조성에서도 나타난다. entity co-occurrence 신호 (Semantic
Entropy $`\rho`$=+0.648, $`p`$=0.043) 와 entity 빈도 신호 (Semantic
Entropy $`\rho`$=+0.636, $`p`$=0.048) 모두 양의 단조성을 보이지만, 두
점수의 평균인 baseline corpus 신호는 Semantic Entropy $`\rho`$=+0.418
($`p`$=0.229) 로 유의 임계 아래에 머무른다
(표 <a href="#tab:axis_decomp" data-reference-type="ref"
...
유의수준 0.05 를 넘지 않는다. 이 신호의 변동 폭은 구간이 올라갈수록
...
일부 구간에서 Semantic Entropy 분리력의 변동이 관찰된다는 정도로 해석을
한정한다.
...
결과 Semantic Entropy 의 변동 폭은 0.178, Semantic Energy 의 변동 폭은
0.162 로, 원래 신호 (Semantic Entropy 0.150, Semantic Energy 0.144) 보다
큰 변동 폭이 관찰되었다. 질문에 한정한 entity 빈도 신호의 Semantic
Entropy 변동 폭은 0.081 로 원래 신호의 0.080 과 거의 같다. 따라서 entity
co-occurrence 단위가 entity 빈도 단위보다 변동 폭이 크다는 본 논문의
결과는 답변 측 entity 를 신호 산출에서 제거해도 유지된다. 비율은 약
1.88배에서 Semantic Entropy 기준 2.20배, Semantic Energy 기준 2.10배로
오히려 증가한다. 이는 두 가지를 시사한다. 첫째, 본 신호의 변동 폭은 모델
출력으로 인해 인공적으로 부풀려진 결과가 아니다. 둘째, 질문 측 entity
co-occurrence이 LLM 의 사전학습된 지식을 더 직접 반영하는 지표일
가능성이 있다. 다만 본 분석의 부분표본은 entity 가 두 개 이상인 질문에
한정된다. 이러한 질문은 단일 사실을 묻기보다 두 entity 사이의 관계를
묻는 지식 질문 유형에 치우친다. 두 entity 사이의 관계는 정의상 entity
co-occurrence과 직접 대응하므로, 본 부분표본은 entity co-occurrence
신호의 변동 폭이 본래 크게 나타나는 조건이며, 이는 *선택 편향*
(selection bias) 에 해당한다. 따라서 비율 2.20 의 절대 크기는 이
부분표본의 특성을 반영한다. 전체 표본의 비율 1.88 과 산술적으로 직접
비교하기보다는, 답변 측 entity 를 제거해도 비율의 부호가 유지된다는
정성적 강건성 결과로 해석한다.
...
co-occurrence 변동 폭 / entity 빈도 변동 폭) 은 1.68 로 전체 표본의 비율
1.88 보다 약 0.20 줄어든다. Semantic Energy 기준 비율도 1.54 로 비슷한
폭으로 감소한다. 두 단위 사이의 부호 (entity co-occurrence \> entity
빈도) 는 유지되지만, 절대 크기는 SVAMP 포함 여부에 따라 달라진다. 본
결과를 토대로 비율은 1.5–1.9 범위로 보고하는 것이 안정적이다.

##### 데이터셋별 분해 결과.

표 <a href="#tab:per_dataset_delta" data-reference-type="ref"
data-reference="tab:per_dataset_delta">[tab:per_dataset_delta]</a> 는
데이터셋별 Semantic Entropy 변동 폭을 entity co-occurrence 신호와 entity
빈도 신호 모두에 대해 보고한다. 다섯 데이터셋 중 네 데이터셋 (BioASQ,
SQuAD-1.1, SVAMP, TriviaQA) 에서는 entity co-occurrence 변동 폭이 entity
빈도 변동 폭보다 크거나 같다. 다만 비율은 TriviaQA 의 1.0배에서 BioASQ
의 2.92배까지 큰 폭으로 변동한다. NQ-Open 에서는 부호가 역전되어 entity
빈도 변동 폭 (0.372) 이 entity co-occurrence 변동 폭 (0.250) 보다 크다.
따라서 전체 표본의 비율 1.88배는 데이터셋별 비율의 평균이 아니라, BioASQ
의 큰 비율 (2.92) 이 NQ-Open 의 역전 (0.67) 을 보상하여 형성된 표본 종합
통계이다. 즉 entity co-occurrence 신호가 모든 데이터셋에서 entity 빈도
신호보다 큰 변동 폭을 보이지는 않는다. 본 결과는 데이터셋 도메인에 따라
적절한 corpus 신호의 단위가 달라질 수 있다는 가설을 제기한다.
...
co-occurrence 신호로 수행하면 Semantic Entropy 기준 변동 폭이
...
탐지 신호를 corpus 조건에 따라 분해하는 분석 도구로 사용했을 때 변동
폭이 $`\Delta`$<!-- -->0.080–0.150 범위에서 관찰된다. 이 비대칭은 corpus
신호의 설계 목표가 달라야 함을 시사한다. 즉 “corpus 신호를 어떻게 더
효과적인 탐지 입력 변수로 가공할 것인가” 가 아니라 “corpus 신호의 어떤
단위가 탐지 신호의 어떤 측면을 분해하는가” 가 본 연구가 제기하는
질문이다.

Entity co-occurrence 신호로 sample 을 나누었을 때의 변동 폭은 단일
entity 빈도 신호로 나누었을 때의 변동 폭보다 약 1.88배 크다 (Semantic
Entropy 기준 $`\Delta`$<!-- -->0.150 와 $`\Delta`$<!-- -->0.080). 이는
두 entity 사이의 관계가 corpus 에 함께 등장하는 정도가 단일 entity 의
등장 횟수보다 LLM 의 사전학습된 지식을 더 직접 반영한다는 해석으로
설명할 수 있다. 어떤 단일 entity 가 corpus 에 자주 등장하더라도 두
entity 사이의 관계가 함께 등장하지 않으면 LLM 이 해당 관계를 학습할
가능성은 낮다. 이러한 단위 차이가 변동 폭의 차이로 나타난다.

Question-answer bridge 신호와 답변 평균 음의 로그우도 (NLL) 사이의 변동
폭 ($`\Delta`$<!-- -->0.176) 은 다른 관점을 제공한다. 질문 entity 와
...
§<a href="#sec:axis" data-reference-type="ref"
data-reference="sec:axis">4.5</a> 에서 보고한 Semantic Entropy 변동 폭
차이의 95% 신뢰구간 \[+0.002, +0.117\] 은 0 을 포함하지 않으나, 하한이
+0.002 로 경계에 가까우며 신뢰구간 폭 (0.115) 이 점추정치 (+0.063) 의 약
1.8배에 해당한다. 즉 “entity co-occurrence 변동 폭이 entity 빈도 변동
폭보다 크다” 는 방향성은 본 표본에서 통계적으로 유의하지만, 효과 크기
자체의 정밀도는 낮다. 약 1.88배라는 점추정치가 모집단 비율과 일치한다고
보지 않으며, 다음 세 가지 점을 함께 보고하였다. 첫째, SVAMP 포함 여부에
따라 비율은 1.68 에서 1.88 사이에서 변동한다. 둘째, 질문 entity 만으로
산출한 강건성 검증에서는 비율이 약 2.20배까지 증가한다. 셋째,
데이터셋별로는 NQ-Open 처럼 부호가 역전되는 경우가 관찰된다
(표 <a href="#tab:per_dataset_delta" data-reference-type="ref"
...
2.  **단일 corpus 색인.** corpus 신호는 `v4_dolmasample_olmo` (16B Dolma
    sample) 단일 Infini-gram 색인에서 산출된다. 이 색인이 Qwen2.5-3B 의
    실제 사전학습 corpus 와 다르므로, corpus 빈도와 환각 사이의 연결은
    상관 관계이며 인과 관계로 해석할 수 없다. 도메인 특화 corpus (예:
    BioASQ 용 PubMed) 를 사용하면 변동 폭이 달라질 가능성이 있다.
...
6.  **변동 폭 차이의 표본 변동성.** 약 1.88배라는 비율은 단일 조건에서
    산출한 점추정치이며, 두 변동 폭의 차이에 대한 sample 단위 bootstrap
    95% 신뢰구간을 §<a href="#sec:axis" data-reference-type="ref"
    data-reference="sec:axis">4.5</a> 에 함께 보고하였다 (Semantic
    Entropy \[+0.002, +0.117\], Energy \[+0.004, +0.109\]). 두 신뢰구간
    모두 0 을 포함하지 않으므로, entity co-occurrence 변동 폭이 entity
    빈도 변동 폭보다 크다는 결과는 본 표본에서 통계적으로 유의하다. 다만
    이 신뢰구간은 단일 모델, 단일 corpus 색인 위에서의 표본 변동성만을
...
8.  **데이터셋별 이질성.**
    표 <a href="#tab:per_dataset_delta" data-reference-type="ref"
    data-reference="tab:per_dataset_delta">[tab:per_dataset_delta]</a>
    에서 보이듯, entity co-occurrence 변동 폭이 entity 빈도 변동 폭보다
    크다는 패턴은 5개 데이터셋 중 NQ-Open 에서 역전된다. 따라서 표본
    종합 비율 1.88배는 모든 데이터셋에서 entity co-occurrence 신호가
    entity 빈도 신호보다 큰 변동 폭을 보인다는 뜻이 아니라, 데이터셋
    종합 통계이다. 도메인별로 corpus 신호의 적절한 단위가 다를 수 있다는
    가설은 본 논문의 범위 밖이다.
...
10. **변동 폭과 Spearman $`\rho`$ 의 분리 해석.** 변동 폭은 max$`-`$min
    통계량이므로, 구간이 올라갈수록 AUROC 가 단조 증가하는 양상과 비단조
    변동 (U자형 등) 을 구분하지 못한다. 본 논문은
    표 <a href="#tab:axis_decomp" data-reference-type="ref"
    data-reference="tab:axis_decomp">[tab:axis_decomp]</a> 에 Semantic
    Entropy / Energy / 답변 평균 NLL 세 신호 모두에 대해 Spearman
    $`\rho`$ 를 함께 보고함으로써 변동 폭과 단조성을 분리해 진단한다.
    다만 21개 동시 검정에 대한 다중비교 보정 (Bonferroni
...
1.  **모델 다양화.** Llama, Gemma, instruction-tuned 모델 등 다양한
    모델에서 entity co-occurrence 신호의 변동 폭 패턴이 같은 방향으로
    나타나는지 검증한다.

2.  **LLM-as-judge 라벨과의 비교.** 본 논문이 사용한 NLI 매칭 라벨을
    LLM-as-judge 라벨로 대체했을 때 corpus 신호 단위별 변동 폭이 어떻게
    달라지는지 분석한다.

3.  **도메인 특화 corpus.** BioASQ 의 경우 PubMed 기반 corpus, 수학
    문제의 경우 ArXiv 기반 corpus 등 도메인 특화 색인을 사용했을 때 동일
    신호의 변동 폭이 더 커지는지 검증한다.

## 결론

본 논문은 환각 탐지 신호 (Semantic Entropy, Semantic Energy, 답변 단위
토큰 logit 통계) 가 corpus 빈도 조건에 따라 어떻게 달라지는지를, corpus
신호의 단위 — 단일 entity, entity co-occurrence, 답변 phrase — 에 따라
비교하는 분석 틀을 제안하였다. Qwen2.5-3B 와 다섯 개 질의응답 데이터셋
(총 35,000 개 생성 답변) 위에서 초기 검증을 수행한 결과, entity
co-occurrence 신호로 sample 을 나누었을 때 Semantic Entropy 의 구간별
AUROC 는 $`\Delta`$<!-- -->0.150 의 변동 폭을 보였다. 이 값은 단일
entity 빈도 신호로 나누었을 때의 변동 폭 $`\Delta`$<!-- -->0.080 의 약
1.5–1.9배에 해당한다 (SVAMP 포함 시 1.88배, 제외 시 1.68배). 두 변동
폭의 차이에 대한 95% 신뢰구간 \[+0.002, +0.117\] 은 본 표본에서 방향성이
통계적으로 유의함을 보이지만, 신뢰구간 폭 대비 점추정치 비율로 보면 효과
크기 자체의 정밀도는 낮다. 모집단 비율의 안정성은 추가 모델과 corpus
위에서의 검증이 필요하다. 동일한 corpus 신호가 fusion 모델의 입력
변수로는 +0.008 의 작은 향상에 한정되는 반면, 분해 신호로 사용했을 때는
큰 변동 폭을 보이는 비대칭은, corpus 신호의 설계 목표를 환각 탐지기의
입력이 아니라 탐지 신호의 조건부 평가 도구로 재정의할 필요를 제기한다.
이러한 재정의는 향후 환각 탐지 연구의 corpus 조건부 평가에서 참고할 수
있는 분석 형식으로 활용될 수 있다.
