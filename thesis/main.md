...
1.  **SE / Energy 의 prompt-level baseline 재현**. Farquhar 등 (2024) 의 SE 와 Ma 등 (2025) 의 Semantic Energy 가 본 데이터 (TruthfulQA + HaluEval-QA, Qwen2.5-3B) 의 prompt-level 환각 탐지에서 어떤 AUROC 를 보이는지 측정하고, SE 원논문 (TriviaQA / Llama-2 7B 기반 AUROC 0.79) 과 일치 여부를 확인한다. 본 RQ 는 단순 replication 이 아니라 후속 RQ2 / RQ3 의 *사전 검증* 이다 — SE / Energy baseline 이 본 데이터에서 재현되지 않으면 그 위에 corpus signal 을 결합한다는 본 논문 framework 자체의 토대가 성립하지 않는다.

2.  **Corpus signal 추가의 한계 효용 (marginal lift) 측정**. SE / Energy 같은 prompt-level uncertainty 신호 위에 외부 corpus support 신호 (entity frequency, entity-pair co-occurrence) 를 추가하면 prompt-level AUROC 가 얼마나 더 올라가는지를 prompt-grouped bootstrap (n=2,000) 으로 통계적 유의성을 포함해 측정한다.
...
##### 본 논문의 contribution 위치 설정.

본 논문의 contribution 은 새로운 결합 알고리즘이 아니라 *외부 corpus 조건에 따른 환각 탐지 신호 신뢰성의 conditional 분포 분석 framework* 이다. Valentin 등 (2024) 은 환각 탐지 점수를 모델 *내부* score attribute (예: top-k logit margin) 에 conditional 하게 calibrate 하는 framework 를 제안하였다. 본 논문은 이와 직교적으로 모델 *외부* corpus statistic 에 conditional 한 lift 분포를 분석한다. 두 framework 는 결합 가능한 보완 신호 구조를 형성하며, 본 논문의 외부 corpus 조건화 분석은 “평균 lift” 라는 단일 수치 뒤에 가려진 영역별 가치 분포를 드러내 환각 탐지 평가의 새 단위를 제시한다.

## 논문 구성

본 논문은 다음과 같이 구성된다. 제 <a href="#ch:related" data-reference-type="ref" data-reference="ch:related">[ch:related]</a>장에서는 LLM 환각 탐지 관련 연구와 본 연구의 위치를 정리한다. 제 <a href="#ch:method" data-reference-type="ref" data-reference="ch:method">[ch:method]</a>장에서는 평가 단위 분리 원칙, prompt-level 평가 단위 채택의 근거, corpus support 를 보완 신호로 쓰는 이론적 근거 (QuCo-RAG, Zhang 등 2025), 그리고 본 논문의 prompt-level fusion 평가 절차를 제시한다. 제 <a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a>장에서는 prompt-level 종합 baseline (SE / Energy / fusion), prompt-grouped bootstrap CI, corpus support bin 별 한계 효용, leave-one-dataset-out 일반화, threshold / calibration 진단, 그리고 더 어려운 candidate-row paired 단위에서의 sensitivity 관찰을 보고한다. 제 <a href="#ch:conclusion" data-reference-type="ref" data-reference="ch:conclusion">[ch:conclusion]</a>장에서 RQ1–3 답변과 학술적 기여, 한계, 향후 연구 방향을 제시한다.
...
SE 의 정의 단위 (prompt-level) 가 본 논문의 main 평가 단위와 일치하므로 본 데이터에서 SE 의 baseline 성능을 직접 측정할 수 있다. SE 단독 prompt-level AUROC 가 Farquhar 등 (2024) 의 원논문 보고치와 일치하는지, 그리고 그 위에 외부 corpus signal 을 추가하면 추가 lift 가 있는지가 본 논문의 핵심 질문이며, 자세한 결과는 §<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a>에서 보고한다.

## Semantic Energy

Ma 등 (2025) 이 제안한 Semantic Energy 는 여러 generated answer 와 semantic cluster 를 사용해 토큰 logit 기반 uncertainty 를 해석하는 방법이다. 본 논문은 Semantic Energy 를 SE 와 함께 prompt-level uncertainty 의 baseline 신호로 사용하며, 두 신호의 본 데이터에서의 baseline AUROC 를 §<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a> 에서 보고한다.
...
### Energy 신호의 motivation 과 본 논문에서의 사용

Energy 의 문헌적 motivation 은 응답 다양성이 작은 사례에서도 토큰 단위 logit 점수가 환각 탐지에 추가 정보를 줄 수 있다는 것이다. 본 논문은 Semantic Energy 를 prompt-level uncertainty baseline 으로 그대로 사용하며 (§<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a> 에서 측정), 그 위에 외부 corpus support 신호를 추가했을 때 발생하는 한계 효용을 측정한다.
...
- **평가 패러다임**. Zhang 등은 단일 생성 답변의 이진 분류 (single answer hallucinated / not) 로 평가하는 반면, 본 연구는 prompt 단위로 “이 prompt 의 free-sample 정답률이 임계값 미만인가” 를 평가하는 prompt-level binary task 를 main 으로 채택한다 (Farquhar 2024 SE 의 평가 단위와 일치). 추가로 같은 prompt 의 정답 후보와 환각 후보를 paired 로 비교하는 candidate-row matched-pair 단위를 sensitivity 분석 (§<a href="#sec:sensitivity" data-reference-type="ref" data-reference="sec:sensitivity">4.5</a>) 으로 보고하여, CHOKE 패턴 (환각 답이 정답 답보다 더 높은 확신으로 평가됨) 을 paired win-rate 로 직접 검출한 결과를 보완 evidence 로 제시한다.
...
Simhi 등 (2025) 은 모델이 정답 지식을 가지고도 높은 확신으로 환각을 생성하는 사례를 CHOKE 로 정의하고, 이런 사례가 logit-only / sample-consistency 기반 detector 의 본질적 한계임을 보였다. 본 데이터의 candidate-row paired 단위에서 CHOKE 패턴이 강하게 관측되며 (NLL paired win-rate 0.38, confidence margin 0.35), 이는 logit / margin 단독 detector 의 한계를 데이터로 보여준다. 본 논문은 이 한계를 직접 메우는 대신, prompt-level 단위에서 SE / Energy 같은 sample-consistency 신호 위에 외부 corpus support 를 보완 결합하는 방향으로 우회한다. 자세한 결과는 §<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a> 에서 보고한다.
...
본 장에서는 본 논문이 제안하는 *prompt-level corpus-conditioned 분석 절차*와 그 이론적 동기를 정리한다. 제 <a href="#ch:related" data-reference-type="ref" data-reference="ch:related">[ch:related]</a>장에서 정리한 SE / Semantic Energy / corpus 신호의 정의 단위 차이와 한계가 출발점이고, 결과 보고 형식 (decile-level corpus 추가 lift heatmap) 까지가 결과 chapter (§<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a>) 와 자연스럽게 연결되도록 절차를 정의한다.
...
##### 이론적 근거 — 두 선행 연구의 경험적 evidence.

Qiu 등 (2025) 의 QuCo-RAG 는 query-aware corpus grounding 에서 entity frequency 와 entity-pair co-occurrence 가 모델 답변의 외부 근거 가능성과 연관됨을 보였다. Zhang 등 (2025) 은 RedPajama 1.3T token pretraining corpus 위에 suffix array 를 구축하고, prompt 와 답변의 n-gram coverage 가 환각 탐지 신호와 비단조적 상관 관계를 가짐을 정량 보고했다. 두 연구의 핵심 시사점은 *corpus exposure 가 모델 출력의 신뢰도 분포를 형성한다* 는 메커니즘 가설이다. 본 논문은 이 가설을 차용하되 corpus statistic 을 직접 detector 로 쓰는 대신, SE / Energy 같은 prompt-level uncertainty 신호 위에 *보완 결합* 하는 방식으로 사용한다.

##### Conditioning 가설의 메커니즘.

“corpus 가 entity 를 거의 모르는 영역에서 corpus 보완 신호의 가치가 가장 클 것” 이라는 본 논문의 conditioning 가설은 두 선행 연구의 결과를 잇는 인과 사슬로 동기 부여된다. (i) Zhao 등 (2024) 의 WildHallucinations 는 *Wikipedia 페이지가 없는 (즉 corpus exposure 가 작은) entity* 에 대한 질문에서 LLM 환각률이 유의미하게 증가함을 관측했다. corpus 부족 → 모델이 해당 entity 에 대한 안정적 representation 을 갖지 못함 → 환각 발생률 ↑. (ii) 그런데 이렇게 발생한 환각은 모델 입장에서 “확신 있게 일관된” 답으로 나오는 경우가 많다는 사실이 Simhi 등 (2025) 의 CHOKE 패턴이다 — 모델은 corpus 가 부족한 영역에서도 free-sample 들이 단일 cluster 로 수렴하는 경향이 있고 (낮은 SE), 이로 인해 SE / Energy 같은 sample-consistency 신호의 환각 판별력이 *구조적으로 약화* 된다. (iii) 따라서 corpus 가 부족한 영역에서는 모델 자체 신호 (SE / Energy) 만으로 hallucination 을 탐지하기 어렵고, 외부 corpus 신호 (entity가 corpus 에서 얼마나 등장하는지) 가 “이 prompt 가 어려운 prompt 임” 을 분리하는 보완 정보를 제공해야 lift 가 발생한다. 본 논문은 이 인과 사슬을 prompt-level corpus support 의 decile 별 corpus 추가 lift 분포로 정량 검증한다 (§<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a> 의 lowest decile lift +0.022 vs high decile lift $`\approx 0`$ 패턴).

##### 운용적 근거.

- **모델 출력에 의존하지 않는 외부 신호**. corpus support 는 모델의 logit, hidden state, 응답 다양성과 독립적으로 산출되므로 SE / Energy 와 직교적이다. 두 신호의 결합이 단순 cumulative 가 아니라 보완적 정보를 갖는다는 가설을 데이터로 검증할 수 있다.

- **Conditioning 가설의 정량 검증 가능성**. corpus support 구간 (3-bin / 5-bin / 10-bin) 별로 corpus 추가의 한계 효용을 측정하면 corpus 신호가 어느 영역에서 가장 가치가 큰지를 데이터로 분리할 수 있다. “corpus 가 entity 를 거의 모르는 영역에서는 모델 자체 신호 (SE / Energy) 만으로 hallucination 을 잘 탐지하기 어렵고 corpus 의 추가 정보가 가장 필요할 것” 이라는 본 논문의 conditioning 가설을 직접 시험할 수 있다.

- **환각 라벨이나 retrieval trigger 와의 분리**. corpus 빈도가 작다고 해서 곧 환각이라는 뜻은 아니고, corpus 빈도가 크다고 해서 곧 정답이라는 뜻도 아니다. 본 논문에서 corpus support 는 detector 가 아니라 SE / Energy 같은 모델 신호 위에 추가되는 *보완 feature* 이다.
...
1.  **Prompt-level 단위를 main 평가 단위로 채택**. SE / Semantic Energy 가 정의상 prompt-level 신호이므로, 이들과 직접 비교 가능한 단위에서 평가해야 단순한 broadcast artifact 없이 fair comparison 이 가능하다. 라벨은 SE 원논문 (Farquhar 2024) framing 그대로 — 각 prompt 의 free-sample N=10 정답 매칭 비율로 binary hard / easy prompt 라벨 부여.

2.  **Single-signal baseline 위에 corpus signal 보완 결합**. SE / Energy / logit-diagnostic 단독 AUROC 와, 동일 feature set 위에 corpus support aggregate (entity_frequency_axis, entity_pair_cooccurrence_axis 의 mean / max / delta) 를 추가한 fusion (LR / RF / GBM / SVM) 의 AUROC 를 비교한다.

3.  **Corpus 추가의 한계 효용을 prompt-grouped bootstrap (n=2,000) 으로 통계 검정 + 구간별 분해**. corpus support 3-bin / 5-bin / 10-bin 별로 fusion (with corpus) vs fusion (no corpus) 의 AUROC delta 를 측정해 corpus 가치의 분포를 드러낸다.

4.  **Sensitivity 보고**. 더 어려운 candidate-row paired 단위에서도 conditioning 효과가 작동하는지 §<a href="#sec:sensitivity" data-reference-type="ref" data-reference="sec:sensitivity">4.5</a> 에서 1 페이지 분량으로 관찰된 사실 위주로 요약한다.

이러한 분석 절차는 SE / Energy 단독 baseline 위에서 corpus signal 추가의 *한계 효용 (marginal lift)* 을 통계적 유의성과 함께 측정하고, 그 효용이 corpus support 영역에 따라 어떻게 분포하는지를 데이터로 보여준다. 실험 결과는 §<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a> 에서 보고한다.
...
후보 답변에 등장하는 entity 의 corpus 내 frequency 와 두 entity 사이의 co-occurrence 를 측정해 corpus support 신호를 만든다. Entity 추출은 *spaCy `en_core_web_lg` NER* 을 사용하며, PERSON / ORG / GPE / LOC / DATE / EVENT / WORK_OF_ART / FAC / NORP / PRODUCT / LANGUAGE / LAW 12 개 label 만 corpus 조회 대상으로 유지한다 (CARDINAL / ORDINAL / MONEY / PERCENT / QUANTITY / TIME 은 factoid 답변의 의미 단위로 부적합하여 제외). NER 이 비면 spaCy noun-chunk 를 fallback 으로 사용하고, 그래도 비면서 “Delhi”, “1941” 처럼 6 단어 이하 짧은 답인 경우 정규화한 텍스트 자체를 단일 entity 로 추가한다. 모든 entity 는 lowercase 정규화 + 공백 압축 + 중복 제거를 거치며 후보 당 최대 8 개로 잘라낸다. Entity-pair 는 한 후보 답변 내에서 추출된 entity 의 unordered pair 로 정의하며, entity 가 2 개 미만 추출된 답변은 co-occurrence 계산에서 제외된다.

corpus count backend 로는 Liu 등(2024) 이 제안한 Infini-gram 을 사용하며, 본 실험은 외부 corpus 로 16B token 규모의 `v4_dolmasample_olmo` 인덱스 (OLMo-7B-hf tokenizer) 를 사용한다. 한 후보 답변에 대해 (i) 추출된 entity 의 frequency, (ii) 두 entity 의 co-occurrence (Infini-gram `count_cnf` AND query), (iii) 이 두 값을 결합한 coverage score 위에서 *rank-quantile 균등 분할* 로 산출한 corpus support 구간 label (낮음 / 중간 / 높음 3-bin, 5-bin, 10-bin) 을 함께 산출한다. rank-quantile 분할은 spaCy NER 채택 후 entity_frequency_axis 분포가 좌편향 (약 58% 가 0) 인 환경에서 fixed-cutoff (0.1, 0.2, ... 절대 임계값) 의 표본 쏠림 문제를 해소하기 위한 선택이다 — 자세한 동기와 한계는 §결론 한계 항목에 정리한다.
...
본 절은 prompt-level 평가 단위에서 단일 signal baseline (SE, Energy, logit-diagnostic, corpus-axis-only) 과 결합기 (LR / RF / GBM / SVM, with / without corpus) 의 AUROC / AUPRC / Brier / ECE 를 표 <a href="#tab:current_thesis_evidence" data-reference-type="ref" data-reference="tab:current_thesis_evidence">4.2</a> 에 보고한다. 라벨은 각 prompt 의 free-sample N=10 정답 매칭 비율 (token-overlap $`\ge 0.5`$) 이 50% 미만이면 *hard prompt* 로 정의한다 (HaluEval-QA 20% / TruthfulQA 97%, 종합 31%; TruthfulQA 라벨 비대칭은 §한계 항목에서 다룬다).
...
- **단일 corpus signal 은 SE / Energy 보다 약함**. corpus-axis-only AUROC 0.634, entity-frequency-only 0.676, entity-pair-cooccurrence-only 0.593 으로 단독 사용 시 SE / Energy 에 미치지 못한다. corpus signal 의 가치는 detector 자체로서가 아니라 SE / Energy 위에 추가되는 보완 신호로서 발휘됨을 시사한다.

- **본 논문 main: GBM (with corpus) 가 종합 1위**. AUROC 0.892으로 Energy 단독 (0.862) 대비 +0.030, SE 단독 (0.832) 대비 +0.060 의 lift. Brier 0.110, ECE 1.7% 로 calibration 도 모든 결합기 중 1위.

- **Corpus 추가의 한계 효용은 결합기 종류에 따라 다름**. LR (no corpus 0.876 → with corpus 0.878), RF (0.888 → 0.889), GBM (0.889 → 0.892) 모두에서 corpus 추가 lift 가 양수이나 절대값은 작다 (+0.002–0.003). 이 lift 의 통계적 유의성과 corpus support 영역별 분포는 다음 두 절 (§Bootstrap CI, §Corpus support 구간별 한계 효용) 에서 자세히 분석한다.
...
표 <a href="#tab:bootstrap_ci" data-reference-type="ref" data-reference="tab:bootstrap_ci">4.3</a> 의 결과는 본 논문 RQ2 (corpus signal 추가의 한계 효용) 에 대한 직접적 답이다 — *tree-based fusion 위에서 corpus signal 의 한계 효용은 아주 작지만 통계적으로 유의* 하다. 다음 절에서는 이 lift 가 corpus support 영역별로 어떻게 분포하는지를 분해해, lift 가 작은 평균값 뒤에 가려진 conditional 가치를 드러낸다.

## Corpus support 구간별 한계 효용 분포

종합 평균 +0.003 의 corpus 추가 lift 가 모든 corpus support 영역에서 균일한지, 아니면 특정 영역에서 더 큰지를 3-bin / 5-bin / 10-bin 분해로 측정한다. 표 <a href="#tab:bin10_method_matrix" data-reference-type="ref" data-reference="tab:bin10_method_matrix">4.4</a> 는 10-bin (rank-quantile decile) × 핵심 4개 method 의 AUROC matrix 이다. 그림 <a href="#fig:money" data-reference-type="ref" data-reference="fig:money">4.1</a> 는 같은 데이터의 reliability map (색=AUROC) 이다.
...
Method $`\times`$ corpus support decile prompt-level AUROC (rank-quantile 분할, 각 decile $`\approx`$ 580 prompts). 굵은 글씨는 decile 별 1위. 마지막 열 = corpus 추가 lift = GBM(with corpus) $`-`$ GBM(no corpus).

</div>

<figure id="fig:money" data-latex-placement="htbp">

<figcaption>Prompt-level AUROC by corpus support decile (n=5<span>,</span>815, 라벨 = is_hard prompt). 색 = AUROC (RdYlGn, 0.5–1.0 범위). corpus 가 풍부할수록 (오른쪽) 모든 결합기 AUROC 가 올라가며, corpus 추가의 한계 효용은 corpus 가 부족한 좌측 (decile 00–10) 에서 가장 크다.</figcaption>
</figure>

핵심 발견은 세 가지이다.

- **Lowest decile 에서 corpus lift 가 가장 큼** (decile 00–10, n=582). GBM (with corpus) 0.851 vs GBM (no corpus) 0.829, lift = +0.022 — 본 논문 conditioning 가설 “corpus 가 entity 를 거의 모르는 영역에서 corpus 추가 정보의 가치가 가장 크다” 의 직접 데이터 evidence. SE / Energy 만으로는 충분히 잡지 못하는 hallucination 패턴이 이 영역에 집중되어 있고, corpus signal 이 그 격차를 메운다.

- **Mid–high decile 에서는 corpus 추가가 큰 차이 없음 또는 미세 negative**. decile 40–80 영역에서 corpus 추가 lift 는 $`-0.009`$   $`0.000`$ 사이. 가능한 해석은 세 가지이다 — (a) 이 영역의 hallucination 은 SE / Energy 의 sample-consistency 신호만으로도 잘 탐지되어 (AUROC 0.85–0.92) corpus aggregate 가 추가 정보를 거의 보태지 못한다; (b) 이 영역에서 corpus aggregate (mean entity_frequency_axis) 의 분포가 평탄해 변별력이 가장 낮다; (c) 효과 크기 ($`-0.005`$ 수준) 가 bootstrap noise 범위 내일 가능성이 있다 (per-bin CI 는 본 논문에서 보고하지 않으나 종합 lift CI 폭이 $`\pm 0.003`$ 수준임을 고려하면 부분적 noise 일 수 있다). 어느 해석이든 corpus 신호의 가치가 lowest decile 에 집중된다는 본 논문의 main 결론을 뒤집지는 않는다.
...
종합하면 *corpus signal 의 가치는 corpus 가 가장 부족한 영역에 집중* 된다. 평균 +0.003 의 lift 뒤에 가려진 conditional 분포를 정확히 보여주는 것이 본 분석의 의의이다. 본 결과는 본 논문 RQ3 (corpus 추가의 한계 효용 영역별 분포) 에 직접 답한다.

## Candidate-level paired 단위에서의 sensitivity 관찰

본 절은 prompt-level 평가 단위가 아닌, 같은 prompt 의 (정답, 환각) 두 후보를 직접 비교하는 *candidate-row paired* 단위 (n=11,630 rows / 5,815 prompts) 에서 동일 결합기들이 어떻게 작동하는지 관찰된 사실 위주로 요약한다. SE / Semantic Energy 는 정의상 prompt-level 신호이므로 이 단위에서는 같은 prompt 의 두 후보에 동일 값이 broadcast 되어 candidate-row AUROC 가 구조적으로 0.5 로 묶인다 (artifact). 이 단위에서의 결합기 비교는 본 논문의 main 결과가 아니라 sensitivity / robustness 관찰로 해석한다.

- **Candidate-level CHOKE 패턴 직접 관찰**. 같은 prompt 의 환각 후보가 정답 후보보다 NLL 이 작은 비율 61.6% (paired win-rate 0.384), confidence margin 이 작은 비율 65.2% (paired win-rate 0.348). Simhi 등 (2025) 이 보고한 high-certainty hallucination 패턴 — 모델이 환각 답을 정답 답보다 더 자신있게 생성하는 현상 — 이 본 데이터에서 강하게 재현된다.

- **Corpus-level paired 신호는 약함**. entity-pair co-occurrence 가 환각 후보 쪽에서 더 큰 비율은 paired win-rate 0.493 (n=2,908) 으로 사실상 무작위. corpus 신호는 candidate 단위 직접 marker 로 쓰기에는 약하다. 이 사실이 본 논문의 main framing — corpus 신호를 prompt-level 보완 결합기에 사용 — 의 합리성을 보강한다.

- **Candidate-row 단위 결합기 비교**. SE / Energy artifact 를 빼고 나면 이 단위에서 GBM (no corpus) 의 candidate-row AUROC 가 0.541 으로 약한 detector 였다. corpus signal 을 추가한 결합기 (corpus-bin weighted fusion) 가 이 단위에서도 AUROC 0.578 로 +0.037 의 lift 를 보였으나, 이 단위 결과는 prompt-level 단위에서의 main 결과 (GBM with corpus 0.892, Energy 단독 대비 +0.030 / SE 단독 대비 +0.060) 를 가리는 것이 아니라 더 어려운 단위에서도 corpus signal 의 기여 방향이 일관됨을 시사하는 robustness 관찰이다.

본 절의 인사이트: (i) CHOKE 패턴이 candidate-level 에서 명확히 관측되어 logit / margin 기반 단독 detector 의 한계를 데이터로 보여주고, (ii) 그 한계를 corpus signal 로 *직접* 메우려 하면 candidate paired 단위에서 corpus 신호가 약하다는 사실을 보지만, (iii) 평가 단위를 prompt-level 로 올리면 corpus 신호의 보완 가치가 통계적으로 유의하게 측정된다. 따라서 본 논문은 prompt-level 단위를 main 으로 채택한 정당성을 데이터로 확인한다.

## 데이터셋 간 일반화

본 분석의 prompt-level 결과가 두 dataset (HaluEval-QA / TruthfulQA) 의 어디에서 어떻게 작동하는지 확인하기 위해 per-dataset AUROC 를 표 <a href="#tab:lodo" data-reference-type="ref" data-reference="tab:lodo">4.5</a> 에 보고한다.
...
Per-dataset prompt-level AUROC (5-fold CV pooled). Eval HaluEval-QA: HaluEval-QA prompt 만으로 평가. Eval TruthfulQA: TruthfulQA prompt 만으로 평가. AGG: 종합.
...
TruthfulQA 의 is_hard 라벨 비율 (0.97) 이 매우 비대칭임에 주의해야 한다. 본 논문의 token-overlap 기반 매칭 (free-sample $`\ge`$ 50% token overlap with right_answer / best_answer / correct_answers list) 은 의미적으로 동등한 paraphrase 를 충분히 잡지 못해 TruthfulQA 의 다양한 정답 표현 (예: “Cardiff”, “the Welsh capital”, “a city in Wales”) 을 hard 로 잘못 분류한다. 따라서 TruthfulQA 단독 AUROC 0.963 은 라벨 노이즈에 의해 부풀려진 수치로 해석해야 하며, *본 논문 main 결과 해석은 HaluEval-QA 단독 (0.817) 또는 AGG (0.892) 기준* 으로 진행한다. NLI-기반 매칭으로 라벨링하면 이 노이즈가 줄어들 가능성이 높으며, 이는 §결론 한계 / 향후 연구 항목에서 명시한다. 그럼에도 GBM (with corpus) 가 두 dataset 모두에서 안정적인 1위라는 사실은 본 논문 main 결과가 단일 dataset 에 의존하지 않음을 보여준다.

## Threshold / calibration 진단

분류 정확도뿐 아니라 출력 점수를 확률로 해석할 수 있는지도 함께 점검하였다 (표 <a href="#tab:calibration" data-reference-type="ref" data-reference="tab:calibration">4.6</a>). Brier score와 ECE(expected calibration error)는 둘 다 작을수록 calibration이 좋다.
...
GBM (with corpus) 는 분류 정확도뿐 아니라 calibration 에서도 강력하다 — Brier 0.110 으로 모든 방법 중 1위, ECE 1.7% 로 GBM (no corpus, 1.4%) 다음 두 번째. AUROC 0.892 + ECE 1.7% 는 prompt-level hallucination detection 의 실용 배포 수준에 가까운 calibration quality 이다 — 즉 출력 점수를 확률로 해석해 임계값 기반 선별 (예: $`\ge 0.7`$ 인 응답만 사용자에게 표시) 이나 downstream 리스크 계산에 사용할 경우 실제 오류율과의 오차가 약 1.7%p 이내임을 의미한다. ranking 만 사용하는 응용에서도 본 결합기의 AUROC 우위가 그대로 활용된다.
...
##### RQ1: SE / Energy 의 prompt-level baseline 재현.

Semantic Entropy 는 prompt-level AUROC 0.832, Semantic Energy 는 0.862으로 측정되었다. 이는 Farquhar 등 (2024) 이 TriviaQA / Llama-2 7B 에서 보고한 SE AUROC 0.79 와 동일 수준이며, 본 데이터 (TruthfulQA + HaluEval-QA, Qwen2.5-3B) 에서 두 prompt-level uncertainty 신호가 강한 baseline 임을 재확인한다. 본 논문이 채택한 SE / Energy 구현이 원논문과 일치하는 baseline 임이 검증된다.

##### RQ2: Corpus signal 추가의 한계 효용.

SE / Energy / logit diagnostic 등 candidate-level / prompt-level 신호 위에 외부 corpus support 신호 (entity frequency / pair co-occurrence aggregate) 를 추가한 GBM (with corpus) 가 prompt-level AUROC 0.892으로 종합 1위이다. 이는 Energy 단독 (0.862) 대비 +0.030, SE 단독 (0.832) 대비 +0.060 의 lift 이며, corpus 추가 자체의 한계 효용은 prompt-grouped bootstrap (n=2,000) 에서 통계적으로 유의함이 확인된다 — 동일 feature set 위에서 corpus 를 추가하면 선형 baseline 대비 +0.0158 \[0.0108, 0.0208\], 비선형 baseline 대비 +0.0027 \[0.0001, 0.0053\] 의 AUROC delta. Calibration 도 1위 (Brier 0.110, ECE 1.7%).

##### RQ3: Corpus 추가 lift 의 영역별 분포.

종합 평균 +0.003 의 corpus 추가 lift 는 corpus support 영역에 따라 균일하지 않다. corpus 가 entity 를 거의 모르는 lowest decile (00–10, n=582) 에서 lift 가 +0.022 로 가장 크고, 이는 본 논문의 *conditioning 가설* (corpus 가 부족한 영역에서 모델 자체 신호 SE/Energy 의 신뢰도가 떨어지므로 corpus 보완 신호의 가치가 가장 크다) 의 직접 데이터 증거이다. 매우 풍부한 영역 (decile 90–100) 에서는 SE / Energy 만으로도 AUROC 0.98 에 도달해 corpus 의 한계 효용이 0 에 가까워지며, 중간 영역 (decile 40–80) 에서는 corpus 추가가 0 또는 미세 negative 효과를 보인다. corpus 노출 빈도와 모델 confidence 사이의 *인과 관계* 해석에는 주의가 필요하다 — 본 실험 corpus (Infini-gram 16B token) 와 Qwen2.5-3B 의 실제 pretraining corpus 가 다를 수 있으므로 본 결과는 *상관* 이지 *인과* 가 아니다 (Zhang 등 (2025) 에서도 동일한 caveat).

## 학술적 기여

본 논문의 학술적 기여는 다음과 같다.

1.  **Corpus signal 추가의 한계 효용을 SE / Energy 위에서 정량 검증**. Farquhar 등 (2024) 의 SE 와 Ma 등 (2025) 의 Semantic Energy 를 prompt-level baseline 으로 재현하고 (각각 AUROC 0.832/ 0.862, 원논문 보고치와 일치), 그 위에 외부 corpus support 신호를 추가한 GBM (with corpus) 가 SE 단독 대비 +0.060, Energy 단독 대비 +0.030 의 lift 를 보임을 prompt-grouped bootstrap (n=2,000) 의 95% CI 와 함께 검증하였다 (corpus 추가의 한계 효용 +0.0027 \[0.0001, 0.0053\], CI lower bound $`> 0`$ 으로 통계적 유의). Calibration 도 1위 (Brier 0.110, ECE 1.7%). 본 결과는 Qiu 등 (2025) (QuCo-RAG) 와 Zhang 등 (2025) 이 제시한 “corpus statistic 이 환각 탐지에 보완 가치를 가진다” 는 가설을 prompt-level uncertainty 신호 위에서 정량 검증한 것이다.

2.  **Corpus 신호 가치의 영역별 분포 측정**. corpus 추가의 평균 +0.003 lift 가 corpus support 영역별로 균일하지 않음을 10-bin decile 분해로 드러냈다. 특히 *lowest decile (00–10) 에서 lift 가 +0.022 로 가장 크다* — corpus 가 entity 를 거의 모르는 영역에서 모델 자체 신호 (SE / Energy) 의 신뢰도가 떨어지므로 corpus 보완 신호의 가치가 가장 크다는 본 논문 *conditioning 가설*의 직접 데이터 evidence 이다. 단일 평균 수치 뒤에 가려진 conditional 가치를 정밀하게 보고하는 분석 절차의 가치를 보여준다.

3.  **평가 단위 분리 + 외부 조건화 분석 framework**. SE 는 prompt-level, Semantic Energy 는 cluster $`\to`$ prompt collapse, logit diagnostic 은 candidate-level 신호임을 본문, 표, 그림 전체에 일관되게 적용하여 단위 혼동에서 비롯된 잘못된 비교를 차단하였다. 본 논문의 framework 는 Valentin 등 (2024) 이 제안한 *내부 score attribute 기반 conditional calibration* 과 직교적으로, 모델 출력에 의존하지 않는 *외부 corpus statistic 기반 conditional 분석* 을 제시한다. 두 framework 는 결합 가능한 보완 구조를 형성하며, 본 논문의 외부 corpus 조건화는 “평균 lift” 라는 단일 수치 뒤에 가려진 영역별 가치 분포 (decile 별 corpus 추가 lift 분포) 를 드러내 환각 탐지 평가의 새 단위를 제시한다.

## 한계

본 연구는 다음 한계를 가진다.

1.  **Prompt-level 라벨의 token-overlap proxy 한계**. 본 논문의 is_hard 라벨은 free-sample N=10 의 정답 token-overlap 매칭 ($`\ge 50\%`$) 을 사용한다. HaluEval-QA 의 짧은 factoid 답변에서는 잘 작동하나 (is_hard 비율 0.20), TruthfulQA 처럼 정답 표현이 다양한 데이터셋에서는 token-overlap 매칭이 의미적으로 동등한 paraphrase 를 잡지 못해 라벨이 hard 쪽으로 치우친다 (TruthfulQA is_hard 비율 0.97). NLI-기반 매칭으로 라벨링하면 이 노이즈가 줄어들 가능성이 높으며, 향후 연구에서 검토한다.

2.  **단일 corpus snapshot 의존**. corpus support 축은 `v4_dolmasample_olmo` (16B Dolma sample tokens, OLMo tokenizer) 단일 index 위에서 산출되므로, 규모나 도메인이 다른 corpus index 에서는 entity frequency 분포와 zero co-occurrence 비율이 달라질 수 있다. 또한 본 실험 corpus 와 Qwen2.5-3B 의 실제 pretraining corpus 가 다르므로 corpus support 와 환각의 연결은 *상관* 이지 *인과* 가 아니다 (Zhang 등 2025 와 동일한 caveat).

3.  **Corpus 신호의 proxy 성격**. corpus-grounded 신호는 외부 근거 가능성을 나타내는 proxy 일 뿐, 모델의 내부 지식을 직접 증명하지 않는다.

4.  **단일 모델 평가**. 본 실험은 Qwen2.5-3B 한 종류의 causal LM 에서만 수행되었고, 모델 규모나 계열에 따른 일반화는 본 논문의 범위를 넘는다.

5.  **모델 내부 상태 해석 불가**. SE, Semantic Energy, corpus feature 모두 출력에서 계산한 관측 지표이며, 모델의 의도나 내부 원인을 직접 추론하지는 않는다.

6.  **Corpus 추가 lift 의 평균 크기는 작음**. 종합 prompt-level AUROC delta = +0.003 (vs GBM no corpus) 으로, 통계적 유의 (CI lower bound $`> 0`$) 하지만 효과 크기는 작다. 본 논문의 main 가치는 (i) lift 가 corpus 가 부족한 영역에 집중된다는 conditional 분포 자체의 발견과 (ii) SE / Energy 를 prompt-level baseline 으로 명시적으로 재현해 fair comparison framework 를 제시한 것에 있다.
...
3.  **모델 일반화**. 동일 평가 절차를 Llama, Mistral 계열 등 다른 causal LM 에 적용해 SE / Energy 의 prompt-level baseline 과 corpus 추가 lift 가 모델에 의존하는지 본다.

4.  **외부 SOTA 환각 탐지 시스템과의 비교**. SelfCheckGPT, INSIDE, FactScore 등 prompt-level / generation-level 환각 탐지 시스템과 본 논문 GBM (with corpus) 의 절대 성능 비교.
