...
Simhi 등(2025)은 모델이 정답 지식을 가지고도 높은 확신으로 환각을 생성하는 사례를 CHOKE로 정의하고, 이런 사례가 logit-only / sample-consistency 기반 detector의 본질적 한계임을 보였다. 본 연구는 Simhi가 제안한 hidden-state probing 대신, corpus support를 조건 축으로 사용해 같은 현상을 외부 신호로 분리해 낼 수 있는지 검증한다. 본 실험에서 CHOKE 패턴은 candidate-level(NLL/margin 의 방향 역전)에서 강하게 재현되며, corpus-level(entity-pair co-occurrence)의 직접 marker 효과는 본 데이터에서 약하게 나타난다. 다만 corpus support 를 *조건 축*으로 활용한 corpus-bin weighted fusion 은 모든 decile 에서 통계적 우위를 보여, conditioning 변수로서의 가치는 강하게 입증된다. 자세한 결과는 §<a href="#ch:experiment" data-reference-type="ref" data-reference="ch:experiment">[ch:experiment]</a>에서 보고한다.
...
후보 답변에 등장하는 entity 의 corpus 내 frequency 와 두 entity 사이의 co-occurrence 를 측정해 corpus support 신호를 만든다. Entity 추출은 *spaCy `en_core_web_lg` NER* 을 기본으로 사용하며, PERSON / ORG / GPE / LOC / DATE / EVENT / WORK_OF_ART / FAC / NORP / PRODUCT / LANGUAGE / LAW 12 개 label 만 corpus 조회 대상으로 유지한다 (CARDINAL / ORDINAL / MONEY / PERCENT / QUANTITY / TIME 은 factoid 답변의 의미 단위로 부적합하여 제외). NER 이 비면 spaCy noun-chunk 를 fallback 으로 사용하고, 그래도 비면서 “Delhi”, “1941” 처럼 6 단어 이하 짧은 답인 경우 정규화한 텍스트 자체를 단일 entity 로 추가한다. 모든 entity 는 lowercase 정규화 + 공백 압축 + 중복 제거를 거치며 후보 당 최대 8 개로 잘라낸다. 초기 시도였던 (a) 정규식 기반 phrase 후보기 (legacy) 와 (b) QuCo-extractor-0.5B (knowledge triplet 모델, Qiu 등 2025 의 distilled 모델) 는 어댑터로 보존하되 default 에서는 사용하지 않는다 — 짧은 factoid 답에서 triplet 추출이 비는 비율이 약 63% 였다. Entity-pair 는 한 후보 답변 내에서 추출된 entity 의 unordered pair 로 정의하며, entity 가 2 개 미만 추출된 답변은 co-occurrence 계산에서 제외된다.
...
<table>
<caption>종합 baseline 결과 (단일 자동화 실행 산출, spaCy entity extractor + rank-quantile corpus bin). 후보 행 11<span>,</span>630개, 학습 라벨은 데이터셋 annotation의 <code>is_hallucination</code>. learned fusion 계열은 prompt-grouped fold 분리로 평가하여 같은 prompt의 두 후보가 fold를 가로지르지 않게 강제하였다.</caption>
<thead>
<tr>
...
<td style="text-align: left;">무작위 수준 (단독 신호 약함)</td>
</tr>
<tr>
...
<tr>
<td colspan="5" style="text-align: left;"><em>Global fusion (with corpus)</em></td>
</tr>
<tr>
<td style="text-align: left;">random forest</td>
...
<tr>
<td style="text-align: left;">axis-interaction logistic fusion</td>
<td style="text-align: center;">0.440</td>
<td style="text-align: center;">0.458</td>
<td style="text-align: center;">0.430</td>
<td style="text-align: left;">axis cross-feature 항</td>
</tr>
<tr>
<td style="text-align: left;"><strong>corpus-bin weighted fusion</strong></td>
<td style="text-align: center;"><strong>0.578</strong></td>
...
- 본 논문이 제안하는 corpus-bin weighted fusion 은 AUROC **0.578** 로 모든 baseline 중 1위이다. 정의상 단순한 선형 구조 위에서 corpus support 를 rank-quantile 로 균등 분할한 bin 마다 다른 가중치를 학습하는 것만으로, 비선형 gradient boosting (0.541) 을 +0.037 차이로 능가한다.

- corpus 신호를 추가한 비선형 baseline (RF / GBM / SVM rbf with corpus) 은 0.540–0.569 로 corpus 신호가 비선형 결합기에도 일부 도움이 되지만, 본 논문 제안 결합기의 우위에는 미치지 못한다.

이는 본 논문의 주장을 직접 뒷받침한다. 단일 global fusion으로는 가려지는 신호 가치가, 외부 조건 축에 따라 분리해 평가하면 같은 데이터에서 복원된다.

## Prompt-grouped bootstrap 신뢰구간

corpus-bin weighted fusion이 *각 corpus support 구간에서* 환각 후보를 정답 후보보다 더 위험한 점수로 평가하는지 통계적으로 검정하기 위해, prompt-grouped bootstrap(같은 prompt의 두 후보를 한 단위로 묶어 resample)을 2,000회 수행하여 95% 신뢰구간을 산출하였다. 표 <a href="#tab:bootstrap_ci" data-reference-type="ref" data-reference="tab:bootstrap_ci">4.3</a>는 corpus support decile별 paired win-rate와 mean paired delta(환각 후보 점수 $`-`$ 정답 후보 점수)의 신뢰구간이다. decile 경계는 rank-quantile 균등 분할을 사용해 각 decile 표본 수를 1,163 으로 동일하게 유지하였다 (legacy fixed-cutoff 방식과의 차이 및 절대값 해석 주의는 §결론 한계 항목 참조).

<div id="tab:bootstrap_ci">

| decile | n | paired win-rate \[95% CI\] | mean delta \[95% CI\] |
|:---|:--:|:--:|:--:|
| 00–10 | 1,163 | **0.618 \[0.578, 0.656\]** | **$`+0.063`$ \[$`+0.051`$, $`+0.074`$\]** |
| 10–20 | 1,163 | **0.568 \[0.526, 0.609\]** | **$`+0.046`$ \[$`+0.035`$, $`+0.057`$\]** |
| 20–30 | 1,163 | **0.635 \[0.595, 0.675\]** | **$`+0.062`$ \[$`+0.050`$, $`+0.074`$\]** |
| 30–40 | 1,163 | **0.650 \[0.612, 0.687\]** | **$`+0.042`$ \[$`+0.034`$, $`+0.050`$\]** |
| 40–50 | 1,163 | **0.639 \[0.600, 0.677\]** | **$`+0.034`$ \[$`+0.027`$, $`+0.041`$\]** |
| 50–60 | 1,163 | **0.639 \[0.599, 0.676\]** | **$`+0.036`$ \[$`+0.028`$, $`+0.043`$\]** |
| 60–70 | 1,163 | **0.594 \[0.556, 0.633\]** | **$`+0.024`$ \[$`+0.017`$, $`+0.032`$\]** |
| 70–80 | 1,163 | **0.548 \[0.509, 0.589\]** | $`-0.003`$ \[$`-0.010`$, $`+0.003`$\] |
| 80–90 | 1,163 | **0.552 \[0.514, 0.593\]** | $`-0.003`$ \[$`-0.009`$, $`+0.003`$\] |
| 90–100 | 1,163 | **0.590 \[0.550, 0.631\]** | **$`+0.012`$ \[$`+0.005`$, $`+0.018`$\]** |

Corpus support decile별 corpus-bin weighted fusion의 paired 통계 95% 신뢰구간 (prompt-grouped bootstrap, n=2,000). win-rate $`> 0.5`$ 가 “환각 후보가 더 위험하게 평가됨”을 의미. spaCy entity extractor + rank-quantile 균등 분할 (각 decile n=1,163).

</div>

세 가지 패턴이 두드러진다.

- **모든 10 decile 에서 통계적으로 유의한 우위**: rank-quantile 분할로 각 decile 표본이 1,163 으로 균등화된 결과, paired win-rate 95% 신뢰구간이 *전 decile 에서 0.5 위에 위치* 한다. corpus 가 거의 모르는 매우 희귀한 영역 (decile 00–10) 에서도, 매우 풍부한 영역 (decile 90–100) 에서도 결합기가 일관되게 환각 후보를 정답 후보보다 더 위험하게 평가한다 (legacy fixed-cutoff 분할에서는 decile 00–10 에 표본의 $`{\sim}81\%`$ 가 몰려 표본 불균형 artifact 가 결과를 왜곡했으며, 본 변화의 동기는 §결론 한계의 “Rank-quantile 분할의 한계와 절대값 해석 주의” 항목 참조).

- **Decile 20–60 sweet spot (n=4,652, 40%)**: 네 decile 모두 win-rate 0.635–0.650 으로 가장 강하다. 중간 corpus support 영역 — corpus 가 entity 의 일부는 알지만 모두 알지는 못하는 “경계 영역” — 에서 본 결합기의 conditional 가중치가 가장 효과적으로 작동한다.

- **Decile 70–90 약화 영역 (mean delta CI 가 0 을 포함)**: win-rate 자체는 0.548 / 0.552 로 통계적 유의 (양 끝 CI lower $`> 0.5`$) 하지만 mean delta CI 가 0 을 포함하여 평균적 점수 격차는 작다. 매우 풍부한 corpus support 영역에서 결합기 우위가 “순서 역전 빈도” 형태로만 남고 점수 격차로는 완화되는 패턴.

종합 수준의 비교에서 corpus-bin weighted fusion (AUROC 0.578) 은 선형 global fusion (logistic regression, AUROC 0.372 — 무작위 0.5 이하의 역방향 결합기) 대비 AUROC **+0.206 \[+0.196, +0.216\]**, AUPRC +0.134 \[+0.126, +0.142\] 로 신뢰구간이 0을 포함하지 않는다. 비선형 sklearn gradient boosting (no corpus, AUROC 0.541) 과의 비교에서도 +0.037 차이로 본 논문 제안 결합기가 종합 1위이다. 본 논문 제안의 우위는 (i) decile 별 paired 분해에서 *모든 10 구간* 통계적 우위 (본 절 표 <a href="#tab:bootstrap_ci" data-reference-type="ref" data-reference="tab:bootstrap_ci">4.3</a>), (ii) calibration 지표의 약 2.2배 개선 (Brier 0.246 vs 0.278, ECE 5.4% vs 12.1%), (iii) leave-one-dataset-out 격차 0.033 (HaluEval-QA 0.587 / TruthfulQA 0.554) 으로 비선형 baseline (RF/SVM with corpus) 의 0.080–0.089 대비 도메인 전이에서도 robust 한 점 (§<a href="#sec:lodo" data-reference-type="ref" data-reference="sec:lodo">4.6</a>) 이 결합된 결과이다. 본 절의 decile 별 분해는 그 조건부 우위가 *어느 corpus support 구간에서 가장 강하게 발생하는지* 를 정밀하게 보여준다.
...
|:---|:--:|:--:|:--:|:--:|:--:|
| 00–10 | 1,163 | **0.635** | 0.596 | 0.586 | 0.395 |
| 10–20 | 1,163 | **0.605** | 0.547 | 0.528 | 0.348 |
| 20–30 | 1,163 | **0.641** | 0.553 | 0.516 | 0.349 |
| 30–40 | 1,163 | **0.596** | 0.546 | 0.518 | 0.332 |
| 40–50 | 1,163 | **0.622** | 0.567 | 0.552 | 0.344 |
| 50–60 | 1,163 | **0.597** | 0.560 | 0.523 | 0.338 |
| 60–70 | 1,163 | **0.579** | 0.561 | 0.550 | 0.367 |
| 70–80 | 1,163 | **0.599** | 0.514 | 0.502 | 0.351 |
| 80–90 | 1,163 | **0.565** | 0.500 | 0.484 | 0.394 |
| 90–100 | 1,163 | **0.571** | 0.517 | 0.507 | 0.464 |

Method $`\times`$ corpus support decile AUROC (spaCy + rank-quantile 기반). 굵은 글씨는 decile별 1위.
...
| decile | n | corpus-bin weighted fusion | gradient boosting (no corpus) | random forest (no corpus) | logit-diagnostic |
|:---|:--:|:--:|:--:|:--:|:--:|
| 00–10 | 1,163 | **0.618** | 0.579 | 0.581 | 0.368 |
| 10–20 | 1,163 | **0.568** | 0.551 | 0.505 | 0.332 |
| 20–30 | 1,163 | **0.635** | 0.534 | 0.521 | 0.334 |
| 30–40 | 1,163 | **0.650** | 0.555 | 0.525 | 0.316 |
| 40–50 | 1,163 | **0.639** | 0.560 | 0.582 | 0.325 |
| 50–60 | 1,163 | **0.639** | 0.563 | 0.548 | 0.303 |
| 60–70 | 1,163 | **0.594** | 0.545 | 0.564 | 0.322 |
| 70–80 | 1,163 | 0.548 | **0.550** | 0.515 | 0.341 |
| 80–90 | 1,163 | **0.552** | 0.502 | 0.503 | 0.387 |
| 90–100 | 1,163 | **0.590** | 0.538 | 0.515 | 0.421 |

Method $`\times`$ corpus support decile paired win-rate (spaCy + rank-quantile 기반). 0.5가 무작위. 굵은 글씨는 decile별 1위.
...
- **Decile 20–60: sweet spot** (n=4,652, 40%). corpus-bin weighted fusion 이 AUROC 0.596–0.641, paired win-rate 0.635–0.650 으로 가장 강하게 작동한다. corpus 가 entity 의 일부는 알지만 모두 알지는 못하는 “경계 영역” 에서 본 결합기의 conditional 가중치가 가장 효과적이다.

- **전 decile 통계적 우위 일관성** (n=11,630, 10/10 구간). rank-quantile 균등 분할 결과, corpus-bin weighted fusion 의 paired win-rate 가 *모든 10 decile 에서 0.5 위 95% CI* 에 위치한다. 양 끝 (decile 00–10, 90–100) 에서도 win-rate 0.59–0.62 를 유지하여, corpus 가 거의 모르는 영역과 매우 잘 아는 영역 모두에서 결합기가 정방향으로 일관 작동한다.

- **Decile 70–80 일시적 dip** (n=1,163). corpus-bin weighted fusion 의 win-rate 가 0.548 로 양 인접 영역 (60–70: 0.594, 80–90: 0.552) 보다 낮고 GBM no corpus (0.550) 와 비슷한 수준까지 떨어진다. 이 영역에서는 corpus 신호 자체가 다른 결합기들과 중첩되어 conditional 가중치의 한계 효용이 작아짐을 시사.

- **Logit-diagnostic CHOKE 패턴**. logit-diagnostic 의 paired win-rate 가 decile 20–60 에서 0.303–0.334 로 가장 강한 역전을 보인다 (decile 20–30: 0.334, 30–40: 0.316, 40–50: 0.325, 50–60: 0.303). 환각 답이 정답 답보다 더 자신있게 생성되는 현상이 *corpus 가 부분적으로 아는* 중간 영역에서 가장 심화되는 패턴 — 모델이 “익숙해 보이지만 사실은 부정확한” 조합을 high-certainty 로 생성한다는 CHOKE 의 전형적 형태이다.

종합하면, corpus support 축은 단순한 “희귀 vs 빈출” 이분법이 아니라, 어느 신호의 신뢰도가 높은가가 지지도에 따라 비단조적으로 바뀌는 축이다. rank-quantile 균등 분할 위에서 본 논문 결합기는 *모든 10 구간* 통계적으로 유의한 우위를 보이며, 그 강도는 중간 영역 (decile 20–60) 에서 정점을 이룬다. 본 논문의 corpus 조건부 reliability 분석이 단일 global fusion 으로 환원되지 않는다는 직접 증거이다.

## CHOKE 패턴 재현 검증 (candidate-level 강함, corpus-level 약함)

Simhi 등(2025)의 CHOKE 패턴, 즉 모델이 정답 지식을 가지고도 high-certainty로 환각을 생성하는 현상을, 본 데이터에서 서로 직교적인 두 평가 scope 에서 검증하였다. candidate-level scope 에서는 강하게 재현되었으나, corpus-level scope 에서는 직접 marker 효과가 약하게 나타나 corpus support 의 가치가 conditioning 변수에 있음을 시사한다.

- **Candidate-level 증거 (강함)**. 환각 답이 정답 답보다 NLL 이 작은 비율 61.6% (paired win-rate, hallu가 더 높을 비율 0.384), confidence margin 이 작은 비율 65.2% (paired win-rate 0.348, n$`\approx`$<!-- -->5,785, ties 제외). 두 신호 모두 “정답이 더 자신있는 답이어야 한다”는 직관과 반대 방향이며, 환각 답이 모델 입장에서 더 자신있는 답으로 평가된다는 뜻이다. 한편 logit_variance 의 paired win-rate 는 0.632 로 정방향 — 환각 답이 더 큰 logit 분산을 보이는 영역에서는 detector 가 정상 작동.

- **Corpus-level 증거 (약함, 본 데이터에서)**. 후보 답에서 추출한 entity-pair co-occurrence 축에서 환각 후보가 정답 후보보다 큰 값을 가질 paired win-rate 가 0.493 (n=2,908, hi=1,435 / lo=1,473) 으로 사실상 무작위에 가깝다. spaCy NER 채택 후 candidate 당 entity 수가 늘어 두 paired 후보의 entity-pair 집합 차이가 작아진 결과로 해석된다. 본 데이터에서 corpus-level CHOKE 직접 증거는 *검출되지 않았으며*, corpus support 의 가치는 직접 marker 로서가 아니라 conditional fusion 의 *conditioning 변수* 로서 작동한다 (본 chapter §종합 베이스라인 결과 및 §Bootstrap CI 참조).

따라서 본 데이터에서 CHOKE 는 candidate-level 신호 (NLL, margin) 의 *방향 역전* 으로 분명히 관측되며, 이는 logit / sample-consistency 기반 detector 의 본질적 한계를 보여준다. corpus support 는 이 한계를 *직접 분리* 하는 marker 가 아니라, candidate 단위 신호의 신뢰도가 어느 corpus support 영역에서 살아나는지를 *조건화* 하는 도구로 사용해야 한다. 본 chapter §종합 베이스라인 결과의 corpus-bin weighted fusion 우위 (AUROC 0.578, GBM no corpus 대비 +0.037) 가 그 conditioning 효과의 정량적 증거이다.
...
|:-------------------------------|:---------:|:----------------:|:---------------:|
| **corpus-bin weighted fusion** | **0.578** |    **0.587**     |    **0.554**    |
...
corpus-bin weighted fusion 은 종합 AUROC 0.578 로 모든 방법 중 1위이며, HaluEval-QA 0.587 / TruthfulQA 0.554 로 두 분할 사이 격차가 *0.033* 으로 gradient boosting no corpus (격차 0.028) 다음으로 두 번째로 작다. 다만 GBM no corpus 의 절대 수준 (AGG 0.541) 이 본 결합기 (0.578) 에 비해 +0.037 낮고, 양 도메인 모두에서 더 약하다 — 즉 GBM 의 작은 격차는 “양쪽 모두 약하게 작동” 의 산물에 가깝다. 비선형 baseline 인 random forest (with corpus, 격차 0.080) 와 SVM rbf (with corpus, 격차 0.089) 는 corpus 신호를 사용하면 HaluEval-QA 에서 더 강하지만 절대 수준에서도 본 논문 결합기에 미치지 못하고, 도메인 전이에서도 격차가 두 배 이상 크다. logit-diagnostic-only 는 격차가 0.122 로 가장 크고 두 데이터셋에서 정반대 방향 — HaluEval-QA 의 환각 후보가 더 강하게 confident 하게 평가되어 logit 신호 역전이 심하고 (AUROC 0.340), TruthfulQA 에서는 그 정도가 약하다 (0.462). corpus-bin weighted fusion 은 corpus 조건에 따라 가중치를 달리 적용하므로 이 dataset-specific CHOKE 강도 차이를 흡수하면서도 두 도메인 모두에서 가장 강한 단일 결합기로 작동한다.
...
| Method                         |   Brier   |    ECE    |
|:-------------------------------|:---------:|:---------:|
| **corpus-bin weighted fusion** | **0.246** |   0.054   |
| SVM rbf (with corpus)          |   0.256   | **0.036** |
...
corpus-bin weighted fusion 은 분류 정확도뿐 아니라 calibration 에서도 강력하다 — Brier 0.246 으로 모든 방법 중 1위, ECE 5.4% 로 SVM rbf (with corpus, ECE 3.6%) 다음 두 번째이다. 단순 baseline (gradient boosting no corpus 의 ECE 12.1%) 대비 약 2.2배 개선. 다만 calibration 우위의 실용적 의미는 downstream use case 에 따라 달라진다 — 본 실험에서 본 논문 결합기는 AUROC 0.578 로 baseline 대비 분명한 우위가 있으나, 다른 방법들은 AUROC 0.5–0.57 범위로 discriminability 자체가 제한적이다. 탐지 점수를 단순 순위 (ranking) 로 쓰는 응용에서는 calibration 의 직접적 이점이 작지만, 점수를 확률로 해석해 임계값 기반 선별 (예: 점수 $`\ge 0.7`$ 인 응답만 사용자에게 표시) 이나 downstream 리스크 계산에 사용하는 경우 calibration 이 좋은 점수는 실제 오류율과의 일치도가 높아 의사결정 품질이 개선된다. 본 실험의 ECE 5.4% 는 실제 오류율과의 오차가 약 5.4%p 이내임을 의미한다.
...
SE / Semantic Energy / logit diagnostic 의 신뢰도는 corpus support 구간에 따라 단조롭지 않은 패턴을 보인다. rank-quantile 균등 분할 (각 decile n=1,163) 위에서 corpus-bin weighted fusion 은 *모든 10 decile 에서* paired win-rate CI 가 0.5 위에 위치하며, 특히 중간 영역 (decile 20–60, 데이터의 40%) 에서 win-rate 0.635–0.650 의 sweet spot 을 형성한다. 동일 영역에서 logit diagnostic 의 win-rate 는 0.303–0.334 까지 떨어져 CHOKE 방향 역전이 가장 심하다 — 모델이 corpus 가 부분적으로 아는 entity 조합을 high-certainty 로 잘못 생성하는 패턴이 데이터로 분리된다. 양 끝 decile (00–10, 90–100) 에서도 결합기가 win-rate 0.59–0.62 로 일관 작동하여, “희귀 vs 빈출” 양 극단 모두에서 정방향으로 작동함이 확인된다. SE / Semantic Energy 자체는 prompt 또는 cluster 단위 신호이므로 candidate-row AUROC 로 직접 비교할 수 없으며, 이를 별도 평가 단위로 분리하지 않으면 신호 가치가 가려진다.

##### RQ2: Conditional fusion의 효용 검증.

선형 global fusion (logistic regression, AUROC 0.372) 은 무작위 0.5 이하의 *역방향* 결합기이다. corpus-bin weighted fusion (AUROC 0.578) 은 이 baseline 대비 AUROC 0.206 \[0.196, 0.216\] 의 유의미한 향상을 달성한다 (AUPRC 차이도 양수, 절대값 0.134). 비선형 sklearn gradient boosting (no corpus, 0.541) 과의 비교에서도 +0.037 차이로 본 논문 결합기가 종합 AUROC 1위이다. 본 논문 결합기의 강점은 (i) Bootstrap CI 가 0 을 포함하지 않는 통계적 유의 우위, (ii) Brier 1위 (0.246) 및 ECE 2위 (5.4%, SVM rbf with corpus 의 3.6% 다음; gradient boosting no corpus 의 12.1% 대비 약 2.2배 개선), (iii) leave-one-dataset-out 격차 0.033 (HaluEval-QA 0.587 / TruthfulQA 0.554) 으로 비선형 baseline (RF/SVM with corpus) 의 0.080–0.089 대비 도메인 전이에서도 robust 한 점이다.

##### RQ3: CHOKE 패턴의 외부 신호 분리.

Simhi 등(2025)의 CHOKE 패턴이 본 데이터의 candidate-level scope 에서 강하게 재현되었다. NLL paired win-rate 0.38, confidence margin 0.35 로 환각 답이 정답 답보다 모델 입장에서 더 자신있는 답으로 평가됨이 일관 관측된다. 한편 corpus-level scope (entity-pair co-occurrence) 에서는 spaCy NER 채택 후 paired win-rate 가 0.493 (n=2,908) 으로 사실상 무작위에 가깝게 나타나, 본 데이터에서 corpus-level CHOKE 의 *직접 marker* 증거는 약하다. 다만 corpus-level 신호는 *conditional fusion 의 conditioning 변수* 로서 본 결합기의 모든 10 decile 통계적 우위를 가능하게 하는 핵심 요인으로 작동한다. corpus 노출 빈도와 모델 confidence 사이의 *인과 관계* 해석에는, 본 실험 corpus(Infini-gram 16B token) 와 모델의 실제 pretraining 데이터가 다르다는 점에서 주의가 필요하다 (Zhang 등(2025) 에서도 동일한 caveat 명시).

## 학술적 기여

본 논문의 학술적 기여는 다음과 같다.

1.  **CHOKE 패턴의 candidate-level 강한 재현 및 conditioning 활용**. Simhi 등(2025)이 hidden-state probing으로 보고한 high-certainty hallucination 패턴을 candidate-level(NLL / confidence margin paired win-rate 0.38 / 0.35) 에서 강하게 재현하였다. corpus-level entity-pair co-occurrence 자체는 본 데이터에서 직접 marker 로 작동하지 않으나 (paired win-rate 0.493), corpus support 를 *조건 축*으로 재해석한 corpus-bin weighted fusion 이 모든 10 decile 에서 통계적으로 유의한 우위를 보여 conditioning 활용 가치는 강하게 입증되었다. Zhang 등(2025)은 독립적으로 pretraining corpus 의 n-gram 통계가 환각 탐지에 보완 신호를 제공함을 보였으며, 본 연구와의 구체적 차이는 §<a href="#ch:related" data-reference-type="ref" data-reference="ch:related">[ch:related]</a>에 정리하였다.

2.  **Conditional reliability 분석 절차의 통계적 검증**. corpus support bin마다 어떤 신호가 안정적으로 작동하는지 분리 평가하는 절차를 제안하고, corpus-bin weighted fusion이 선형 global fusion 대비 AUROC 0.206 \[0.196, 0.216\] 의 유의미한 우위를 보임을 95% prompt-grouped bootstrap으로 검증하였다. 비선형 gradient boosting (no corpus) 대비 +0.037 AUROC 우위, calibration 에서 Brier 1위 (0.246) 및 ECE 2위 (5.4%, SVM rbf with corpus 의 3.6% 다음) — 이는 단순 baseline (gradient boosting no corpus 의 ECE 12.1%) 대비 약 2.2배 개선이다 — , 그리고 LODO 도메인 격차 0.033 (대비 SVM/RF 의 0.080–0.089) 의 세 측면 모두에서 corpus 조건부 구조의 독립적 가치를 확인하였다.
...
5.  **Rank-quantile 분할의 한계와 절대값 해석 주의**. 본 논문은 fixed-cutoff binning (entity_frequency_axis 의 0.1, 0.2, ... 절대 임계값) 이 spaCy entity extractor 채택 후 데이터 분포 좌편향 (58% 가 axis $`= 0`$) 으로 인해 decile 00–10 에 81% 가 몰리는 문제를 해결하기 위해 rank-quantile 균등 분할 (각 decile n=1,163) 로 전환하였다. 이로써 decile 비교의 통계적 power 는 균등화되었으나, 각 decile 의 “절대 corpus support 강도” 는 데이터 분포에 따라 달라진다 (예: decile 50–60 의 entity_frequency_min cutoff 가 다른 데이터셋에서는 다른 절대값에 해당). decile 라벨을 절대 corpus 빈도로 직접 해석하기보다 *본 데이터 내 상대 순위* 로 읽어야 한다.
