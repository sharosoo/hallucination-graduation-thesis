...
- **Logit-diagnostic CHOKE 패턴**. logit-diagnostic 의 paired win-rate 가 decile 30–60 에서 0.303–0.334 로 가장 강한 역전을 보인다. 환각 답이 정답 답보다 더 자신있게 생성되는 현상이 *corpus 가 부분적으로 아는* 중간 영역에서 가장 심화되는 패턴 — 모델이 “익숙해 보이지만 사실은 부정확한” 조합을 high-certainty 로 생성한다는 CHOKE 의 전형적 형태이다.
