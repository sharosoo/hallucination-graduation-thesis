...
선형 global fusion (logistic regression, AUROC 0.372) 은 무작위 0.5 이하의 *역방향* 결합기이다. corpus-bin weighted fusion (AUROC 0.578) 은 이 baseline 대비 AUROC 0.206 \[0.196, 0.216\] 의 유의미한 향상을 달성한다 (AUPRC 차이도 양수, 절대값 0.134). 비선형 sklearn gradient boosting (no corpus, 0.541) 과의 비교에서도 +0.037 차이로 본 논문 결합기가 종합 AUROC 1위이다. 본 논문 결합기의 강점은 (i) Bootstrap CI 가 0 을 포함하지 않는 통계적 유의 우위, (ii) calibration 1위 (Brier 0.246, ECE 5.4% — gradient boosting 의 1/2.2 수준), (iii) leave-one-dataset-out 격차 0.033 (HaluEval-QA 0.587 / TruthfulQA 0.554) 으로 비선형 baseline (RF/SVM with corpus) 의 0.080–0.089 대비 도메인 전이에서도 robust 한 점이다.
...
2.  **Conditional reliability 분석 절차의 통계적 검증**. corpus support bin마다 어떤 신호가 안정적으로 작동하는지 분리 평가하는 절차를 제안하고, corpus-bin weighted fusion이 선형 global fusion 대비 AUROC 0.206 \[0.196, 0.216\] 의 유의미한 우위를 보임을 95% prompt-grouped bootstrap으로 검증하였다. 비선형 gradient boosting (no corpus) 대비 +0.037 AUROC 우위 + calibration ECE 5.4% vs 12.1% (약 2.2배 개선) + LODO 도메인 격차 0.033 (대비 SVM/RF 의 0.080–0.089) 의 세 측면 모두에서 corpus 조건부 구조의 독립적 가치를 확인하였다.
...
Y. Zhang et al., “Probing Hallucination via Pretraining-corpus Coverage: A Suffix-Array N-gram Study,” *arXiv preprint arXiv:2511.17946*, 2025.
