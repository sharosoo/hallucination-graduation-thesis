...
단어의 빈도보다 “Paris” 와 “France” 가 corpus 에서 함께 등장한 빈도와 더
직접적으로 연결될 것이다. 그렇다면 환각 탐지 신호의 신뢰도 또한 단일
entity 빈도가 아니라 entity 쌍의 동시 등장 빈도에 따라 더 분명하게 갈릴
것이다.
...
환각 탐지 신호의 신뢰도를 약 1.5–1.9배 더 명확하게 구분함을 정량
...
entity co-occurrence 신호로 sample 을 10분위로 나눌 경우, Semantic
Entropy 의 AUROC range ($`\Delta`$) 는 0.150, Semantic Energy 는 0.144
로 나타났다. 이는 본 연구에서 비교한 일곱 가지 corpus 신호 가운데 가장
큰 값이다.

sample 단위 부트스트랩 (500회 반복) 으로 계산한 95% 신뢰구간은 Semantic
Entropy AUROC range 가 0.121–0.211, entity 빈도 신호는 0.058–0.149 였다.
두 값의 차이는 95% 신뢰구간 \[+0.002, +0.117\] 으로 0 을 포함하지
