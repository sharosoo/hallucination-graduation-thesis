...
(2023)[@kandpal2023longtail] 이 사전학습 corpus 의 (질문 entity, 답변
entity) 쌍 동시 등장 횟수가 단일 entity 빈도보다 LLM 의 사실 정답률을 더
잘 예측한다는 것을 정량적으로 보인 결과와 직접 연결된다. 그렇다면 환각
탐지 신호의 신뢰도 역시 단일 entity 빈도가 아니라 entity 쌍의 동시 등장
빈도에 따라 더 뚜렷하게 갈릴 것이다.
...
- \(2\) Kandpal 등 (2023)[@kandpal2023longtail] 이 모델 정답률에 대해
  ...
  신뢰구간 하한 +0.002, SVAMP 제외 시 1.68배). 측정 대상이 *모델 정답률*
  에서 *환각 탐지 신호의 AUROC* 로 한 단계 위층으로 옮겨가도 같은 방향이
  유지된다는 점이 본 확인의 핵심이다.
...
정량 연구로 Kandpal 등 (2023)[@kandpal2023longtail] 과 Mallen 등
(2023)[@mallen2023whennot] 이 본 연구의 직접적 선행이다. Kandpal 등
(2023) 은 The Pile 과 ROOTS 사전학습 corpus 에서 (질문 entity, 답변
...
보였다. WildHallucinations[@wildhallucinations2024] 는 Wikipedia
페이지가 없는 entity 에 대한 질문에서 LLM 의 환각률이 유의미하게
증가한다는 점을 관찰하여, corpus 노출 부족과 환각 발생률 사이의 경험적
연관성을 보고했다. Zhang 등 (2025)[@zhang2025corpus] 은 RedPajama 1.3 조
토큰 corpus 위에 suffix array 를 구축하고, prompt 와 답변의 n-gram
빈도를 환각 탐지 신호로 평가했다.
...
AUROC* 로 한 단계 위층으로 옮긴다. 두 양은 자동으로 따라오지 않는다.
정답률이 높은 영역과 낮은 영역에서도 Semantic Entropy 의 AUROC 는 동일할
수 있고, 정답률이 균일한 영역에서도 AUROC 가 영역마다 다를 수 있다. 또한
동일 corpus 신호의 두 역할 --- 탐지기 입력 변수와 영역별 평가 척도 ---
사이의 비대칭은 두 선행 연구에는 정의 자체가 없는 분석 축이다. 본 연구가
일곱 가지 corpus 신호 단위 (entity 빈도, entity co-occurrence, baseline,
question-answer bridge, 답변 3-gram / 5-gram, 3-gram 미등장) 를 같은
평가 척도 위에서 비교한 것도 두 선행 연구의 범위 밖이다.
