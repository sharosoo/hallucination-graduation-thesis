...
reasoning 에서 동일 패턴 (corpus 노출-성능 양의 관계) 이 reasoning
전반에 걸쳐 나타남을 뒷받침한다. 본 연구는 측정 대상을 정답률에서 환각
탐지 신호의 AUROC 로 한 단계 끌어올려 분석한다.
...
질문 텍스트에서만 entity 를 추출하여 재산출한 결과, 비율 (entity
co-occurrence AUROC range / entity 빈도 AUROC range) 은 SE 기준
1.88배에서 2.20배로 증가한다 (n=2,095 부분표본). 다만 부분표본 자체가
entity 두 개 이상의 질문, 즉 entity pair 관계를 묻는 질문에 치우쳐 *선택
편향* 이 존재하므로, 절대 크기를 전체 표본의 1.88 과 직접 비교하기는
어렵다. 답변 측 entity 를 제거해도 비율의 부호가 유지된다는 정성적
강건성 결과로 해석한다.

##### 강건성 검증: SVAMP 제외 민감도.

수학 단어 문제인 SVAMP 를 제외한 3,200 sample 으로 같은 분석을
재수행하면, SE 기준 비율은 1.88 에서 1.68 로 약 0.20 감소하고 Energy
기준도 1.54 로 비슷한 폭으로 줄어든다. 부호는 유지되나 절대 크기는 SVAMP
포함 여부에 따라 달라지며, 안정적 보고 범위는 1.5--1.9배이다.
