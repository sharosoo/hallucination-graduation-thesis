...
\begin{tikzpicture}[
    ...
    label/.style={font=\scriptsize, text=black!60, fill=white, inner sep=1pt}
]
...
% 중단: 게이트와 메트릭 계산 (출처 분리)
\node[decision, below=1.6cm of cluster] (gate) {$|C|=1$?};
\node[energybox, below left=1.8cm and 0.75cm of gate] (energy) {Zero-SE 보완\\Energy 사용\\{\tiny token logits}};
\node[processbox, below right=1.8cm and 0.75cm of gate] (se) {의미 다양성 있음\\SE 사용\\{\tiny cluster distribution}};

% 하단: 출력
\node[outputbox, below=4.0cm of gate] (output) {환각 점수};

% 화살표
\draw[arrow] (input) -- (sample);
\draw[arrow] (sample) -- (cluster);
\draw[arrow] (cluster) -- node[right, label] {클러스터 수 $|C|$} (gate);
\draw[arrow] (gate) -- node[pos=0.35, left=2pt, label] {Yes} (energy);
\draw[arrow] (gate) -- node[pos=0.35, right=2pt, label] {No} (se);
\draw[arrow] (energy) |- (output);
\draw[arrow] (se) |- (output);

% 원천 신호는 각 메트릭 박스 내부의 작은 설명으로 표시하여 화살표/라벨 겹침을 피한다.
...
\begin{axis}[
    ...
    legend style={
        at={(0.5,1.12)}, anchor=south,
        font=\small,
        draw=none, fill=white, fill opacity=0.95,
        legend columns=2,
...
\addplot[fill=primaryblue!70, draw=primaryblue] coordinates {(2,0.61) (3,0.66)};
\addplot[fill=secondaryorange!70, draw=secondaryorange] coordinates {(1,0.74) (2,0.52) (3,0.42)};
\legend{SE, Energy}

\node[font=\scriptsize, text=primaryblue, align=center] at (axis cs:0.86,0.08) {N/A};
\draw[primaryblue!70, line width=0.8pt] (axis cs:0.75,0.035) -- (axis cs:0.97,0.035);
\end{axis}
...
\node[font=\small] at (3, 6.65) {통합 포착률: \textbf{34.1\%}};
...
\addplot[fill=accentgreen!70, draw=accentgreen, bar shift=0pt] coordinates {(3,0.643)};
\end{axis}
...
% Cascade 설명
\node[font=\scriptsize, text=black!80, align=center, anchor=north,
      fill=white, draw=black!20, rounded corners=2pt, inner sep=2pt]
      at (6.8, 0.85) {$|C|=1$: Energy\\그 외: SE};
